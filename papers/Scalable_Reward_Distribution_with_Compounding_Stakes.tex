...
\author{Richard Pardoe \thanks{rick@fluid.org}}
\date{December 2020\\v1.0}
\maketitle
\begin{abstract}
    Scalability of SEIV2 dApps is constrained by the gas costs of writing to storage. 
    ...
    Here we derive formulas for a scalable implementation of a compounding, decreasing Stability Pool deposit and its corresponding SEI gain. Thus, Fluid tracks the deposits and rewards of every depositor with only $O(1)$ complexity for each liquidation.
\end{abstract}

\section{Introduction}
The Fluid protocol uses a Stability Pool as its primary mechanism to liquidate collateralized debt positions (called ``troves'') that fall below the minimum collateralization ratio of 110$\%$ $\cite{Whitepaper}$. Stability deposits (provided in SAI) absorb and cancel the debt (in SAI) from defaulted troves: an amount of SAI in the Stability Pool corresponding to the debt of the liquidated trove is burned from the pool’s balance, offsetting the debt. In return, Stability Pool depositors are rewarded with the acquisition of collateral (in SEI) from liquidated positions at a significant discount. The proportion of a stability depositor’s current deposit to the total SAI in the pool determines the collateral share it receives from the liquidation. 

The fact that liquidated troves decrease all deposits in the Stability Pool leads to an implementation challenge: Due to SEIV2's block gas limit, it becomes impossible to iterate over all deposits in the Stability Pool for a large number of deposits. Such an operation has computational complexity of $O(n)$ and does not scale. 
...
The methods presented in this paper are generalizable and can be used for other dApps running on SEIV2.

\section{Derivation}
For a pull-based implementation in Fluid’s Stability Pool, we must factor out the initial deposit, and track reward terms that are independent from the individual deposit values. We derive formulas for a compounding SAI deposit and corresponding SEI gain as functions of the initial deposit, and involving a product $P$ and a sum $S$: the former is a function of the SAI losses, and the latter is a function of $P$, the SEI rewards, and the total deposits.
...
\textbf{Terms}
\begin{terms}
    \item $d_i:$ A given user’s SAI deposit at liquidation event \textit{i}\\
    \item $Q_i:$ The SAI debt absorbed by the Stability Pool from liquidation \textit{i}\\
    \item $E_i:$ The SEI sent to the Stability Pool from liquidation \textit{i}\\
    \item $D_i:$ Total SAI deposits at liquidation \textit{i}
\end{terms}
...
\bigskip
Liquidation assigns an SAI loss to the deposit proportional to the prior share of total deposits:
...
\bigskip
\subsection{Corresponding SEI Gain}

\bigskip
The SAI deposit earns an SEI gain at each liquidation.
At each liquidation $i$, the user’s SAI deposit effectively decreases. Thus we can write the
user’s cumulative SEI gain $e_{n}$, from a series of \textit{n} liquidations, as:
\begin{equation} 
...
\bigskip
Thus, we have the depositor's cumulative SEI gain $e_n$ as a function of the initial deposit $d_0$, and the rewards and total deposits at each liquidation.\\
Let the summation $\displaystyle\sum^n_{k=1}[...]$ be denoted $S_n$.

\bigskip
\subsection{SEI gain for initial deposit made at $t>0$}
...
\bigskip
We take a snapshot of the sum at liquidation $t$, $S_t$. Then, the SEI gain earned by the deposit at liquidation $n$ is:
\begin{equation} 
...
\bigskip
\underline{Upon each liquidation yielding SAI debt ‘Q’ offset with the Stability Pool and SEI gain ‘E’:}

Update $S$: $S = S + \frac{E}{D}P$ (\textit{intuition: a deposit’s marginal SEI gain is equal to the deposit $\times$ SEI per unit staked $\times$ current correction factor)}\\
Update $P$: $P = P(1 - \frac{Q}{D})$\\
Update total deposits: $D = D - Q$

\bigskip
\underline{Withdrawing the deposit and SEI gain:}

Compute final compounded SAI deposit $d$: $d = d_t\frac{P}{P_t}$\\
Compute final corresponding cumulative SEI gain $e$: $e = d_t\frac{S - S_t}{P_t}$\\
Send $d$ and $e$ to user\\
Update deposit: deposit[user] = 0\\
Update total deposits: $D = D - d$\\

\bigskip
\section{Practical Implementation in Fluid}
...
\bigskip
\textbf{Solution}: Complete pool-emptying is handled by tracking a “current epoch” variable. The SEI gain reward sum $S$ for each epoch is stored in a mapping.
...
When users withdraw, check their epoch snapshot against the current epoch: If equal, deposit and SEI gain are computed as normal.
...
\begin{itemize}
    ...
    \item The user’s \textbf{SEI gain} is computed using the $S$ sum that corresponds to the epoch in which their deposit was made.
\end{itemize}
...
\bigskip
If the number of scale changes is 0, compute the deposit and SEI gain as normal.
...
\bigskip
\textbf{SEI Gain:} Since the deposit may span up to one scale change, so too does the reward. In this case, obtain the SEI gain using the $S$ sums from the two consecutive scales that the deposit spans. The reward from the second sum is scaled by 1e-18. The rewards from both scales are added to make the final SEI gain.
...
\bigskip
This allows us to track the SEI reward terms at each scale and epoch.Thus we can correctly compute the SEI rewards for a compounding decreasing stake, taking account of periodic Pool-emptying, and getting around the limitations of Solidity arithmetic.

\bigskip
All implementation logic above is combined in PoolManager.sol, in the functions:

\bigskip
$\_getCompoundedSAIDeposit()$

$\_getDepositorSEIGain()$
...
\textit{Scalable Reward Distribution on the SEIV2 Blockchain, 2018}. 
...
\bibitem{Whitepaper} 
R. Lauko, R. Pardoe. 
\textit{Fluid: Decentralized Borrowing Protocol (Whitepaper), 2020}. 
