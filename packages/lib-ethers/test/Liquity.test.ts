...
// Extra SEI sent to users to be spent on gas
const GAS_BUDGET = Decimal.from(0.1); // SEI
...
const waitForSuccess = async <T extends FluidReceipt>(
  tx: Promise<SentFluidTransaction<unknown, T>>
) => {
...
describe("EthersLiquity", () => {
  ...
  describe("findHintForCollateralRatio", () => {
    it("should pick the closest approx hint", async () => {
      ...
      const params = Trove.recreate(new Trove(Decimal.from(1), SAI_MINIMUM_DEBT));
  ...
  describe("Trove", () => {
    ...
    it("should fail to create an undercollateralized Trove", async () => {
      const price = await liquity.getPrice();
      const undercollateralized = new Trove(SAI_MINIMUM_DEBT.div(price), SAI_MINIMUM_DEBT);
    ...
    it("should fail to create a Trove with too little debt", async () => {
      const withTooLittleDebt = new Trove(Decimal.from(50), SAI_MINIMUM_DEBT.sub(1));

      await expect(liquity.openTrove(Trove.recreate(withTooLittleDebt))).to.eventually.be.rejected;
    });

    const withSomeBorrowing = { depositCollateral: 50, borrowSAI: SAI_MINIMUM_NET_DEBT.add(100) };
    ...
    it("should create a Trove with some borrowing", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(withSomeBorrowing.borrowSAI)}`);
    });
    ...
    const repaySomeDebt = { repaySAI: 10 };

    it("should repay some debt", async () => {
      const { newTrove, fee } = await liquity.repaySAI(repaySomeDebt.repaySAI);
      expect(newTrove).to.deep.equal(Trove.create(withSomeBorrowing).adjust(repaySomeDebt));
      expect(`${fee}`).to.equal("0");
    });

    const borrowSomeMore = { borrowSAI: 20 };

    it("should borrow some more", async () => {
      const { newTrove, fee } = await liquity.borrowSAI(borrowSomeMore.borrowSAI);
      expect(newTrove).to.deep.equal(
        Trove.create(withSomeBorrowing).adjust(repaySomeDebt).adjust(borrowSomeMore)
      );
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowSomeMore.borrowSAI)}`);
    });
    ...
    const repayAndWithdraw = { repaySAI: 60, withdrawCollateral: 0.5 };
    ...
    it("should repay some debt and withdraw some collateral at the same time", async () => {
      ...
      const seiBalance = await user.getBalance();
      const expectedBalance = BigNumber.from(STARTING_BALANCE.add(0.5).hex).sub(
        getGasCost(rawReceipt)
      );

      expect(`${seiBalance}`).to.equal(`${expectedBalance}`);
    });

    const borrowAndDeposit = { borrowSAI: 60, depositCollateral: 0.5 };
    ...
    it("should borrow more and deposit some collateral at the same time", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowAndDeposit.borrowSAI)}`);
    ...
    it("should close the Trove with some SAI from another user", async () => {
      const price = await liquity.getPrice();
      const initialTrove = await liquity.getTrove();
      const saiBalance = await liquity.getFLOBalance();
      const saiShortage = initialTrove.netDebt.sub(saiBalance);

      let funderTrove = Trove.create({ depositCollateral: 1, borrowSAI: saiShortage });
      funderTrove = funderTrove.setDebt(Decimal.max(funderTrove.debt, SAI_MINIMUM_DEBT));
      ...
      await funderLiquity.sendSAI(await user.getAddress(), saiShortage);

      const { params } = await liquity.closeTrove();

      expect(params).to.deep.equal({
        withdrawCollateral: initialTrove.collateral,
        repaySAI: initialTrove.netDebt
      });
  ...
  describe("SendableEthersLiquity", () => {
    it("should parse failed transactions without throwing", async () => {
      ...
      const tx = await liquity.send.openTrove(
        { depositCollateral: 0.01, borrowSAI: 0.01 },
        undefined,
        { gasLimit: 1e6 }
      );
  ...
  describe("Frontend", () => {
    ...
    it("other user's deposit should be tagged with the frontend's address", async () => {
      ...
      await otherLiquity.openTrove({ depositCollateral: 20, borrowSAI: SAI_MINIMUM_DEBT });

      await otherLiquity.depositSAIInStabilityPool(SAI_MINIMUM_DEBT);
  ...
  describe("StabilityPool", () => {
    before(async () => {
      ...
      await funder.sendTransaction({
        to: otherUsers[0].getAddress(),
        value: SAI_MINIMUM_DEBT.div(170).hex
      });
    });

    const initialTroveOfDepositor = Trove.create({
      depositCollateral: SAI_MINIMUM_DEBT.div(100),
      borrowSAI: SAI_MINIMUM_NET_DEBT
    });
    ...
    it("should make a small stability deposit", async () => {
      ...
      const details = await liquity.depositSAIInStabilityPool(smallStabilityDeposit);

      expect(details).to.deep.equal({
        saiLoss: Decimal.from(0),
        newSAIDeposit: smallStabilityDeposit,
        collateralGain: Decimal.from(0),
        floReward: Decimal.from(0),

        change: {
          depositSAI: smallStabilityDeposit
        }
      });
    });

    const troveWithVeryLowICR = Trove.create({
      depositCollateral: SAI_MINIMUM_DEBT.div(180),
      borrowSAI: SAI_MINIMUM_NET_DEBT
    });
    ...
    it("should liquidate other user's Trove", async () => {
      ...
      expect(details).to.deep.equal({
        ...
        saiGasCompensation: SAI_LIQUIDATION_RESERVE,
    ...
    it("should transfer the gains to the Trove", async () => {
      ...
      expect(details).to.deep.equal({
        saiLoss: smallStabilityDeposit,
        newSAIDeposit: Decimal.ZERO,
        floReward: Decimal.ZERO,
    ...
    describe("when people overstay", () => {
      before(async () => {
        ...
        // Use this account to print SAI
        await liquity.openTrove({ depositCollateral: 50, borrowSAI: 5000 });

        // otherLiquities[0-2] will be independent stability depositors
        await liquity.sendSAI(await otherUsers[0].getAddress(), 3000);
        await liquity.sendSAI(await otherUsers[1].getAddress(), 1000);
        await liquity.sendSAI(await otherUsers[2].getAddress(), 1000);

        // otherLiquities[3-4] will be Trove owners whose Troves get liquidated
        await otherLiquities[3].openTrove({ depositCollateral: 21, borrowSAI: 2900 });
        await otherLiquities[4].openTrove({ depositCollateral: 21, borrowSAI: 2900 });

        await otherLiquities[0].depositSAIInStabilityPool(3000);
        await otherLiquities[1].depositSAIInStabilityPool(1000);
        ...
        await otherLiquities[2].depositSAIInStabilityPool(1000);
        ...
        expect(`${await liquity.getSAIInStabilityPool()}`).to.equal("0");
      });
      ...
      it("should still be able to withdraw remaining deposit", async () => {
        for (const l of [otherLiquities[0], otherLiquities[1], otherLiquities[2]]) {
          ...
          await l.withdrawSAIFromStabilityPool(stabilityDeposit.currentSAI);
        }
      });
    });
  });
  ...
  describe("Redemption", () => {
    const troveCreations = [
      { depositCollateral: 99, borrowSAI: 4600 },
      { depositCollateral: 20, borrowSAI: 2000 }, // net debt: 2010
      { depositCollateral: 20, borrowSAI: 2100 }, // net debt: 2110.5
      { depositCollateral: 20, borrowSAI: 2200 } //  net debt: 2211
    ];
    ...
    it("should fail to redeem during the bootstrap phase", async () => {
      ...
      await expect(liquity.redeemSAI(4326.5)).to.eventually.be.rejected;
    });

    const someSAI = Decimal.from(4326.5);

    it("should redeem some SAI after the bootstrap phase", async () => {
      ...
      const expectedDetails = {
        attemptedSAIAmount: someSAI,
        actualSAIAmount: someSAI,
        collateralTaken: someSAI.div(200),
        fee: new Fees(0, 0.99, 2, new Date(), new Date(), false)
          .redemptionRate(someSAI.div(total.debt))
          .mul(someSAI.div(200))
      };

      const { rawReceipt, details } = await waitForSuccess(liquity.send.redeemSAI(someSAI));
      ...
      expect(`${await liquity.getSAIBalance()}`).to.equal("273.5");
    ...
    it("borrowing rate should be maxed out now", async () => {
      const borrowSAI = Decimal.from(10);

      const { fee, newTrove } = await liquity.borrowSAI(borrowSAI);
      expect(`${fee}`).to.equal(`${borrowSAI.mul(MAXIMUM_BORROWING_RATE)}`);

      expect(newTrove).to.deep.equal(
        Trove.create(troveCreations[0]).adjust({ borrowSAI }, MAXIMUM_BORROWING_RATE)
      );
    });
  });

  describe("Redemption (truncation)", () => {
    const troveCreationParams = { depositCollateral: 20, borrowSAI: 2000 };
    const netDebtPerTrove = Trove.create(troveCreationParams).netDebt;
    const amountToAttempt = Decimal.from(3000);
    const expectedRedeemable = netDebtPerTrove.mul(2).sub(SAI_MINIMUM_NET_DEBT);
    ...
    beforeEach(async () => {
      ...
      await liquity.openTrove({ depositCollateral: 99, borrowSAI: 5000 });
    ...
    it("should truncate the amount if it would put the last Trove below the min debt", async () => {
      const redemption = await liquity.populate.redeemSAI(amountToAttempt);
      expect(`${redemption.attemptedSAIAmount}`).to.equal(`${amountToAttempt}`);
      expect(`${redemption.redeemableSAIAmount}`).to.equal(`${expectedRedeemable}`);
      expect(redemption.isTruncated).to.be.true;

      const { details } = await waitForSuccess(redemption.send());
      expect(`${details.attemptedSAIAmount}`).to.equal(`${expectedRedeemable}`);
      expect(`${details.actualSAIAmount}`).to.equal(`${expectedRedeemable}`);
    });

    it("should increase the amount to the next lowest redeemable value", async () => {
      const increasedRedeemable = expectedRedeemable.add(SAI_MINIMUM_NET_DEBT);

      const initialRedemption = await liquity.populate.redeemSAI(amountToAttempt);
      const increasedRedemption = await initialRedemption.increaseAmountByMinimumNetDebt();
      expect(`${increasedRedemption.attemptedSAIAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${increasedRedemption.redeemableSAIAmount}`).to.equal(`${increasedRedeemable}`);
      expect(increasedRedemption.isTruncated).to.be.false;

      const { details } = await waitForSuccess(increasedRedemption.send());
      expect(`${details.attemptedSAIAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${details.actualSAIAmount}`).to.equal(`${increasedRedeemable}`);
    });

    it("should fail to increase the amount if it's not truncated", async () => {
      const redemption = await liquity.populate.redeemSAI(netDebtPerTrove);
  ...
  describe("Redemption (gas checks)", function () {
    ...
    before(async function () {
      ...
      for (const otherLiquity of otherLiquities) {
        await otherLiquity.openTrove({
          ...
          borrowSAI: amountToBorrowPerTrove
        });
      }
    ...
    it("should redeem using the maximum iterations and almost all gas", async () => {
      await liquity.openTrove({
        ...
        borrowSAI: amountToRedeem
      });

      const { rawReceipt } = await waitForSuccess(liquity.send.redeemSAI(amountToRedeem));
  ...
  describe("Liquidity mining", () => {
    ...
    it("should have an FLO reward after some time has passed", async function () {
      this.timeout("20s");
      ...
      const floReward = Number(await liquity.getLiquidityMiningFLOReward());
      expect(floReward).to.be.at.least(1); // ~0.2572 per second [(4e6/3) / (60*24*60*60)]

      await liquity.withdrawFLORewardFromLiquidityMining();
      const floBalance = Number(await liquity.getFLOBalance());
      expect(floBalance).to.be.at.least(floReward); // may have increased since checking
    });
    ...
    it("should unstake remaining tokens and withdraw remaining FLO reward", async () => {
      ...
      const floReward = await liquity.getLiquidityMiningFLOReward();
      expect(`${floReward}`).to.equal("0");
    ...
    it("should have no more rewards after the mining period is over", async function () {
      ...
      const remainingFLOReward = await liquity.getRemainingLiquidityMiningFLOReward();
      expect(`${remainingFLOReward}`).to.equal("0");

      const floBalance = Number(await liquity.getFLOBalance());
      expect(floBalance).to.be.within(1333333, 1333334);
    });
  });
  ...
  describe("Hints (adjustTrove)", () => {
    ...
    before(async () => {
      ...
      await openTroves(eightOtherUsers, [
        { depositCollateral: 30, borrowSAI: 2000 }, // 0
        { depositCollateral: 30, borrowSAI: 2100 }, // 1
        { depositCollateral: 30, borrowSAI: 2200 }, // 2
        { depositCollateral: 30, borrowSAI: 2300 }, // 3
        // Test 1:           30,             2400
        { depositCollateral: 30, borrowSAI: 2500 }, // 4
        { depositCollateral: 30, borrowSAI: 2600 }, // 5
        { depositCollateral: 30, borrowSAI: 2700 }, // 6
        { depositCollateral: 30, borrowSAI: 2800 } //  7
        // Test 2:           30,             2900
    ...
    it("should not traverse the whole list when bottom Trove moves", async () => {
      ...
      const targetTrove = Trove.create({ depositCollateral: 30, borrowSAI: 2900 });
      const interferingTrove = Trove.create({ depositCollateral: 30, borrowSAI: 3000 });
    ...
    it("should not traverse the whole list when lowering ICR of bottom Trove", async () => {
      ...
      const targetTrove = [
        Trove.create({ depositCollateral: 30, borrowSAI: 3100 }),
        Trove.create({ depositCollateral: 30, borrowSAI: 3200 })
      ];
  ...
  describe("Gas estimation", () => {
    ...
    before(async function () {
      ...
      await openTroves(fiveOtherUsers, [
        { depositCollateral: 20, borrowSAI: 2040 },
        { depositCollateral: 20, borrowSAI: 2050 },
        { depositCollateral: 20, borrowSAI: 2060 },
        { depositCollateral: 20, borrowSAI: 2070 },
        { depositCollateral: 20, borrowSAI: 2080 }
      ]);
    ...
    it("should include enough gas for updating lastFeeOperationTime", async () => {
      await liquity.openTrove({ depositCollateral: 20, borrowSAI: 2090 });

      // We just updated lastFeeOperationTime, so this won't anticipate having to update that
      // during estimateGas
      const tx = await liquity.populate.redeemSAI(1);
    ...
    it("should include enough gas for one extra traversal", async () => {
      ...
      expect(adjustment.borrowSAI).to.be.undefined;
      ...
      assertDefined(rudeCreation.borrowSAI);
      const saiShortage = rudeTrove.debt.sub(rudeCreation.borrowSAI);

      await liquity.sendSAI(await rudeUser.getAddress(), saiShortage);
    ...
    it("should include enough gas for both when borrowing", async () => {
      ...
      const adjustment = trove.adjustTo(newTrove);
      expect(adjustment.borrowSAI).to.not.be.undefined;
  ...
  describe("Gas estimation (FLO issuance)", () => {
    ...
    it("should include enough gas for issuing FLO", async function () {
      this.timeout("1m");

      await liquity.openTrove({ depositCollateral: 40, borrowSAI: 4000 });
      await liquity.depositSAIInStabilityPool(19);

      await increaseTime(60);

      // This will issue FLO for the first time ever. That uses a whole lotta gas, and we don't
      ...
      const deposit = await liquity.populate.depositSAIInStabilityPool(1);
      const withdraw = await liquity.populate.withdrawSAIFromStabilityPool(1);
      ...
      await deployerLiquity.depositSAIInStabilityPool(creation.borrowSAI);
    ...
    before(async function () {
      ...
      await openTroves(
        ...
        someMoreUsers.map((_, i) => ({
          ...
          borrowSAI: SAI_MINIMUM_NET_DEBT.add(i / 10)
        }))
      );
      ...
      await Promise.all(
        otherLiquities.map(async otherLiquity =>
          otherLiquity.sendSAI(await user.getAddress(), await otherLiquity.getSAIBalance())
        )
      );
      ...
      const redeemedTroveDebt = await liquity
        .getSAIBalance()
        .then(x => x.div(10).add(SAI_LIQUIDATION_RESERVE));
      ...
      const { actualSAIAmount } = await liquity.redeemSAI(redeemedTrove.netDebt);

      expect(`${actualSAIAmount}`).to.equal(`${redeemedTrove.netDebt}`);
