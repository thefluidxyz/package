...
* 
*
*  --- Functionality added specific to the FLOToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Fluid contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending FLO directly to a Fluid
* core contract, when they should rather call the right function.
*
* 2) sendToFLOStaking(): callable only by Fluid core contracts, which move FLO tokens from user -> FLOStaking contract.
*
...
*
* 8) (64 + 2/3) million tokens are minted at deployment to the Fluid multisig
*
* 9) Until one year from deployment:
* -Fluid multisig may only transfer() tokens to LockupContracts that have been deployed via & registered in the 
...
* -sendToFLOStaking() reverts when the multisig is the sender, blocking the multisig from staking its FLO.
* 
* After one year has passed since deployment of the FLOToken, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract FLOToken is CheckContract, IFLOToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "FLO";
    string constant internal _SYMBOL = "FLO";
    ...
    address public immutable floStakingAddress;
    ...
    event FLOStakingAddressSet(address _floStakingAddress);
        ...
        public 
    {
        ...
        // Allocate the remainder to the FLO Multisig: (100 - 2 - 32 - 1.33) million = 64.66 million
    ...
    function sendToFLOStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsFLOStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from staking FLO
        _transfer(_sender, floStakingAddress, _amount);
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'FLO: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'FLO: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "FLO: Cannot transfer tokens directly to the FLO token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != floStakingAddress,
            "FLO: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "FLOToken: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "FLOToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "FLOToken: caller must not be the multisig");
    }

    function _requireCallerIsFLOStaking() internal view {
         require(msg.sender == floStakingAddress, "FLOToken: caller must be the FLOStaking contract");
    }
