...
contract('Echidna debugger', async accounts => {
  ...
  before(async () => {
    ...
    saiToken = await SAIToken.at(await echidnaTester.saiToken())
  ...
  it.only('SAI balance', async () => {
    await echidnaTester.openTroveExt('0', '0', '4210965169908805439447313562489173090')

    const totalSupply = await saiToken.totalSupply();
    const gasPoolBalance = await saiToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getSAIDebt();
    const defaultPoolBalance = await defaultPool.getSAIDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalSAIDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = saiToken.balanceOf(currentTrove);
