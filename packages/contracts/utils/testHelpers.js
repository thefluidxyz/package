...
class TestHelper {
  ...
  // To checkRecoveryMode / getTCR from the Fluid mainnet contracts, pass a price value - this can be the lastGoodPrice
  // stored in Fluid, or the current Chainlink ETHUSD price, etc.
  ...
  // Adds the gas compensation (50 SAI)
  ...
  /*
   * given the requested SAI amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, saiAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(saiAmount)
    const compositeDebt = await this.getCompositeDebt(contracts, saiAmount)
    return compositeDebt.add(fee)
  }

  /*
   * given the desired total debt, returns the SAI amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveSAIAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, saiAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(saiAmount)
    return saiAmount.add(fee)
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const SAIAmount = redemptionTx.logs[i].args[0]
        const totalSAIRedeemed = redemptionTx.logs[i].args[1]
        const totalETHDrawn = redemptionTx.logs[i].args[2]
        const ETHFee = redemptionTx.logs[i].args[3]

        return [SAIAmount, totalSAIRedeemed, totalETHDrawn, ETHFee]
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const saiGasComp = liquidationTx.logs[i].args[3]

        return [liquidatedDebt, liquidatedColl, collGasComp, saiGasComp]
      }
    }
  ...
  static getSAIFeeFromSAIBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "SAIBorrowingFeePaid") {
        return (tx.logs[i].args[1]).toString()
      }
    }
    throw ("The transaction logs do not contain an SAIBorrowingFeePaid event")
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const pendingSAIDebtReward = await contracts.troveManager.getPendingSAIDebtReward(account)
    const entireColl = rawColl.add(pendingETHReward)
    const entireDebt = rawDebt.add(pendingSAIDebtReward)
  ...
  static async getCollAndDebtFromWithdrawSAI(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepaySAI(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, SAIChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = SAIChange.gt(this.toBN('0')) ? await contracts.troveManager.getBorrowingFee(SAIChange) : this.toBN('0')
    const newColl = entireColl.add(ETHChange)
    const newDebt = entireDebt.add(SAIChange).add(fee)
  ...
  static async openTrove_allAccounts(accounts, contracts, ETHAmount, SAIAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, SAIAmount)

    for (const account of accounts) {
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, SAIAmount, upperHint, lowerHint, { from: account, value: ETHAmount })
  ...
  static async openTrove_allAccounts_randomETH(minETH, maxETH, accounts, contracts, SAIAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, SAIAmount)
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.openTrove(this._100pct, SAIAmount, upperHint, lowerHint, { from: account, value: randCollAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_randomETH_ProportionalSAI(minETH, maxETH, accounts, contracts, proportion) {
    const gasCostList = []
  
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH)
      const proportionalSAI = (web3.utils.toBN(proportion)).mul(web3.utils.toBN(randCollAmount))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalSAI)

      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, proportionalSAI, upperHint, lowerHint, { from: account, value: randCollAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_randomETH_randomSAI(minETH, maxETH, accounts, contracts, minSAIProportion, maxSAIProportion, logging = false) {
    const gasCostList = []
    const price = await contracts.priceFeedTestnet.getPrice()
    ...
    for (const account of accounts) {
      ...
      const randSAIProportion = this.randAmountInWei(minSAIProportion, maxSAIProportion)
      const proportionalSAI = (web3.utils.toBN(randSAIProportion)).mul(web3.utils.toBN(randCollAmount).div(_1e18))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalSAI)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)
      ...
      if (logging && tx.receipt.status) {
        i++
        const ICR = await contracts.troveManager.getCurrentICR(account, price)
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalSAI} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
  ...
  static async openTrove_allAccounts_randomSAI(minSAI, maxSAI, accounts, contracts, ETHAmount) {
    const gasCostList = []

    for (const account of accounts) {
      const randSAIAmount = this.randAmountInWei(minSAI, maxSAI)
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, randSAIAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, randSAIAmount, upperHint, lowerHint, { from: account, value: ETHAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_decreasingSAIAmounts(accounts, contracts, ETHAmount, maxSAIAmount) {
    const gasCostList = []

    let i = 0
    for (const account of accounts) {
      const SAIAmount = (maxSAIAmount - i).toString()
      const SAIAmountWei = web3.utils.toWei(SAIAmount, 'ether')
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, SAIAmountWei)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, SAIAmountWei, upperHint, lowerHint, { from: account, value: ETHAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
      i += 1
    }
  ...
  static async openTrove(contracts, {
    ...
    if (!extraSAIAmount) extraSAIAmount = this.toBN(0)
    else if (typeof extraSAIAmount == 'string') extraSAIAmount = this.toBN(extraSAIAmount)
    ...
    const saiAmount = MIN_DEBT.add(extraSAIAmount)
    ...
    const tx = await contracts.borrowerOperations.openTrove(maxFeePercentage, saiAmount, upperHint, lowerHint, extraParams)
  ...
  static async withdrawSAI(contracts, {
    maxFeePercentage,
    saiAmount,
    ICR,
    upperHint,
    ...
    assert(!(saiAmount && ICR) && (saiAmount || ICR), "Specify either sai amount or target ICR, but not both")
    ...
    if (ICR) {
      ...
      saiAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt)
    } else {
      increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, saiAmount)
    }

    await contracts.borrowerOperations.withdrawSAI(maxFeePercentage, saiAmount, upperHint, lowerHint, extraParams)
  ...
  static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, SAIAmount) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let SAIChangeBN = this.toBN(SAIAmount)

      const { newColl, newDebt } = await this.getCollAndDebtFromAdjustment(contracts, account, ETHChangeBN, SAIChangeBN)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const zero = this.toBN('0')

      let isDebtIncrease = SAIChangeBN.gt(zero)
      SAIChangeBN = SAIChangeBN.abs() 

      // Add ETH to trove
      if (ETHChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, SAIChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account, value: ETHChangeBN })
      // Withdraw ETH from trove
      } else if (ETHChangeBN.lt(zero)) {
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, SAIChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account })
      }
  ...
  static async adjustTrove_allAccounts_randomAmount(accounts, contracts, ETHMin, ETHMax, SAIMin, SAIMax) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let SAIChangeBN = this.toBN(this.randAmountInWei(SAIMin, SAIMax))
      ...
      // Withdraw ETH from trove
        ...
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, SAIChangeBN, isDebtIncrease, lowerHint,  upperHint,{ from: account })
      }

      const gas = this.gasUsed(tx)
      // console.log(`ETH change: ${ETHChangeBN},  SAIChange: ${SAIChangeBN}, gas: ${gas} `)
  ...
  static async withdrawSAI_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawSAI(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawSAI(this._100pct, amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawSAI_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randSAIAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawSAI(contracts, account, randSAIAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawSAI(this._100pct, randSAIAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repaySAI_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepaySAI(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repaySAI(amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repaySAI_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randSAIAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromRepaySAI(contracts, account, randSAIAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repaySAI(randSAIAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async redeemCollateral(redeemer, contracts, SAIAmount, gasPrice = 0, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice()
    const tx = await this.performRedemptionTx(redeemer, price, contracts, SAIAmount, maxFee, gasPrice)
    const gas = await this.gasUsed(tx)
    return gas
  }

  static async redeemCollateralAndGetTxObject(redeemer, contracts, SAIAmount, gasPrice, maxFee = this._100pct) {
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randSAIAmount)
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async performRedemptionTx(redeemer, price, contracts, SAIAmount, maxFee = 0, gasPrice_toUse = 0) {
    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(SAIAmount, price, gasPrice_toUse)
    ...
    const tx = await contracts.troveManager.redeemCollateral(SAIAmount,
      firstRedemptionHint,
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = []
    for (const account of accounts) {
      const randomSAIAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.provideToSP(randomSAIAmount, this.ZERO_ADDRESS, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomSAIAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.withdrawFromSP(randomSAIAmount, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async getTimeFromSystemDeployment(floToken, web3, timePassedSinceDeployment) {
    const deploymentTime = await floToken.getDeploymentStartTime()
