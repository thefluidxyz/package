...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    const alice_Coll = ((await troveManager.Troves(alice))[1]
      .add(await troveManager.getPendingSEIReward(alice)))
      .toString()
    const carol_Coll = ((await troveManager.Troves(carol))[1]
      .add(await troveManager.getPendingSEIReward(carol)))
      .toString()

    /* Expected collateral:
    A: Alice receives 0.995 SEI from L1, and ~3/5*0.995 SEI from L2.
    expect aliceColl = 2 + 0.995 + 2.995/4.995 * 0.995 = 3.5916 SEI

    C: Carol receives ~2/5 SEI from L2
    expect carolColl = 2 + 2/4.995 * 0.995 = 2.398 SEI

    Total coll = 4 + 2 * 0.995 SEI
    */
    ...
    const entireSystemColl = (await activePool.getSEI()).add(await defaultPool.getSEI()).toString()
    assert.equal(entireSystemColl, A_coll.add(C_coll).add(th.applyLiquidationFee(B_coll.add(D_coll))))

    // check SAI gas compensation
    assert.equal((await saiToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingSEIReward(bob)))
      .toString()
    const dennis_Coll = ((await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingSEIReward(dennis)))
      .toString()
    const erin_Coll = ((await troveManager.Troves(erin))[1]
      .add(await troveManager.getPendingSEIReward(erin)))
      .toString()

    /* Expected collateral:
    A and B receives 1/2 SEI * 0.995 from L1.
    total Coll: 3

    A, B, receive (2.4975)/8.995 * 0.995 SEI from L2.
    
    D, E receive 2/8.995 * 0.995 SEI from L2.

    expect A, B coll  = 2 +  0.4975 + 0.2763  =  SEI
    expect D, E coll  = 2 + 0.2212  =  SEI
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has SEI from all previously liquidated troves", async () => {
    // A, B  open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: bob } })
    ...
    const freddy_SEIReward = (await troveManager.getPendingSEIReward(freddy)).toString()

    /* Expected collateral:
     A-E should have been liquidated
     trove F should have acquired all SEI in the system: 1 SEI initial coll, and 0.995^5+0.995^4+0.995^3+0.995^2+0.995 from rewards = 5.925 SEI
    */
    ...
    const gainedSEI = th.applyLiquidationFee(
    ...
    assert.isAtMost(th.getDifference(freddy_SEIReward, gainedSEI), 1000)

    const entireSystemColl = (await activePool.getSEI()).add(await defaultPool.getSEI()).toString()
    assert.isAtMost(th.getDifference(entireSystemColl, F_coll.add(gainedSEI)), 1000)

    // check SAI gas compensation
    assert.equal((await saiToken.balanceOf(owner)).toString(), dec(1000, 18))
  })
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100000, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100000, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100000, 18), extraParams: { from: C } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(20000, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: D } })
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100000, 18), extraParams: { from: E } })
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    // Check accumulated SEI gain for each trove
    const B_SEIGain_1 = await troveManager.getPendingSEIReward(B)
    const C_SEIGain_1 = await troveManager.getPendingSEIReward(C)
    const D_SEIGain_1 = await troveManager.getPendingSEIReward(D)
    const E_SEIGain_1 = await troveManager.getPendingSEIReward(E)

    // Check gains are what we'd expect from a distribution proportional to each trove's entire coll
    const B_expectedPendingSEI_1 = A_collRedistribution.mul(B_entireColl_0).div(denominatorColl_1)
    const C_expectedPendingSEI_1 = A_collRedistribution.mul(C_entireColl_0).div(denominatorColl_1)
    const D_expectedPendingSEI_1 = A_collRedistribution.mul(D_entireColl_0).div(denominatorColl_1)
    const E_expectedPendingSEI_1 = A_collRedistribution.mul(E_entireColl_0).div(denominatorColl_1)

    assert.isAtMost(getDifference(B_expectedPendingSEI_1, B_SEIGain_1), 1e8)
    assert.isAtMost(getDifference(C_expectedPendingSEI_1, C_SEIGain_1), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingSEI_1, D_SEIGain_1), 1e8)
    assert.isAtMost(getDifference(E_expectedPendingSEI_1, E_SEIGain_1), 1e8)
    ...
    const B_SEIGain_2 = await troveManager.getPendingSEIReward(B)
    const D_SEIGain_2 = await troveManager.getPendingSEIReward(D)
    const E_SEIGain_2 = await troveManager.getPendingSEIReward(E)

    // Since B topped up, he has no previous pending SEI gain
    const B_expectedPendingSEI_2 = C_collRedistribution.mul(B_entireColl_1).div(denominatorColl_2)

    // D & E's accumulated pending SEI gain includes their previous gain
    const D_expectedPendingSEI_2 = C_collRedistribution.mul(D_entireColl_1).div(denominatorColl_2)
      .add(D_expectedPendingSEI_1)

    const E_expectedPendingSEI_2 = C_collRedistribution.mul(E_entireColl_1).div(denominatorColl_2)
      .add(E_expectedPendingSEI_1)

    assert.isAtMost(getDifference(B_expectedPendingSEI_2, B_SEIGain_2), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingSEI_2, D_SEIGain_2), 1e8)
    assert.isAtMost(getDifference(E_expectedPendingSEI_2, E_SEIGain_2), 1e8)
    ...
    const B_SEIGain_3 = await troveManager.getPendingSEIReward(B)
    const D_SEIGain_3 = await troveManager.getPendingSEIReward(D)

    // Since B topped up, he has no previous pending SEI gain
    const B_expectedPendingSEI_3 = E_collRedistribution.mul(B_entireColl_2).div(denominatorColl_3)

    // D'S accumulated pending SEI gain includes their previous gain
    const D_expectedPendingSEI_3 = E_collRedistribution.mul(D_entireColl_2).div(denominatorColl_3)
      .add(D_expectedPendingSEI_2)

    assert.isAtMost(getDifference(B_expectedPendingSEI_3, B_SEIGain_3), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingSEI_3, D_SEIGain_3), 1e8)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: carol } })
    ...
    //Bob adds SEI to his trove
    const addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(bob, bob, { from: bob, value: addedColl })

    // Alice withdraws SAI
    await borrowerOperations.withdrawSAI(th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, { from: alice })
    ...
    // Expect Bob now holds all Ether and SAIDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 SAI (10 each for gas compensation)
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingSEIReward(bob)))
      .toString()

    const bob_SAIDebt = ((await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingSAIDebtReward(bob)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_SAIDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)), 1000)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    // D opens trove
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: dennis } })
    ...
    /* Bob rewards:
     L1: 1/2*0.995 SEI, 55 SAI
     L2: (2.4975/3.995)*0.995 = 0.622 SEI , 110*(2.4975/3.995)= 68.77 SAIDebt
    ...
    debt: 233.77 SAIDebt
     ...
     Alice rewards:
    L1 1/2*0.995 SEI, 55 SAI
    L2 (1.4975/3.995)*0.995 = 0.3730 SEI, 110*(1.4975/3.995) = 41.23 SAIDebt

    coll: 1.8705 SEI
    debt: 146.23 SAIDebt

    totalColl: 4.99 SEI
    totalDebt 380 SAI (includes 50 each for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingSEIReward(bob)))
      .toString()
    ...
    const alice_SAIDebt = ((await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingSAIDebtReward(alice)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
    assert.isAtMost(th.getDifference(bob_SAIDebt, expected_B_debt), 10000)
    ...
    assert.isAtMost(th.getDifference(alice_SAIDebt, expected_A_debt), 10000)
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // A, B, C, D open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ extraSAIAmount: dec(110, 18), extraParams: { from: carol, value: _998_Ether } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: dennis, value: dec(1000, 'ether') } })
    ...
    // Expected rewards:  alice: 1 SEI, bob: 1 SEI, carol: 998 SEI
    const alice_SEIReward_1 = await troveManager.getPendingSEIReward(alice)
    const bob_SEIReward_1 = await troveManager.getPendingSEIReward(bob)
    const carol_SEIReward_1 = await troveManager.getPendingSEIReward(carol)

    //Expect 1000 + 1000*0.995 SEI in system now
    const entireSystemColl_1 = (await activePool.getSEI()).add(await defaultPool.getSEI()).toString()
    ...
    th.assertIsApproximatelyEqual(alice_SEIReward_1.toString(), th.applyLiquidationFee(D_coll).mul(A_coll).div(totalColl))
    th.assertIsApproximatelyEqual(bob_SEIReward_1.toString(), th.applyLiquidationFee(D_coll).mul(B_coll).div(totalColl))
    th.assertIsApproximatelyEqual(carol_SEIReward_1.toString(), th.applyLiquidationFee(D_coll).mul(C_coll).div(totalColl))

    //Carol adds 1 SEI to her trove, brings it to 1992.01 total coll
    const C_addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(carol, carol, { from: carol, value: dec(1, 'ether') })

    //Expect 1996 SEI in system now
    const entireSystemColl_2 = (await activePool.getSEI()).add(await defaultPool.getSEI())
    th.assertIsApproximatelyEqual(entireSystemColl_2, totalColl.add(th.applyLiquidationFee(D_coll)).add(C_addedColl))

    // E opens with another 1996 SEI
    ...
    /* Expected SEI rewards: 
     Carol = 1992.01/1996 * 1996*0.995 = 1982.05 SEI
     Alice = 1.995/1996 * 1996*0.995 = 1.985025 SEI
     Bob = 1.995/1996 * 1996*0.995 = 1.985025 SEI

    therefore, expected total collateral:

    Carol = 1991.01 + 1991.01 = 3974.06
    Alice = 1.995 + 1.985025 = 3.980025 SEI
    Bob = 1.995 + 1.985025 = 3.980025 SEI
    ...
    */
    ...
    //Expect 3982.02 SEI in system now
    const entireSystemColl_3 = (await activePool.getSEI()).add(await defaultPool.getSEI()).toString()
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))

    // check SAI gas compensation
    th.assertIsApproximatelyEqual((await saiToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // Expected rewards:  alice: 1 SEI, bob: 1 SEI, carol: 998 SEI (*0.995)
    const alice_SEIReward_1 = await troveManager.getPendingSEIReward(alice)
    const bob_SEIReward_1 = await troveManager.getPendingSEIReward(bob)
    const carol_SEIReward_1 = await troveManager.getPendingSEIReward(carol)

    //Expect 1995 SEI in system now
    const entireSystemColl_1 = (await activePool.getSEI()).add(await defaultPool.getSEI()).toString()
    assert.equal(entireSystemColl_1, A_coll.add(B_coll).add(C_coll).add(th.applyLiquidationFee(D_coll)))
    ...
    /* Alice, Bob, Carol each adds 1 SEI to their troves, 
    ...
    //Expect 1998 SEI in system now
    const entireSystemColl_2 = (await activePool.getSEI()).add(await defaultPool.getSEI()).toString()
    th.assertIsApproximatelyEqual(entireSystemColl_2, totalColl.add(th.applyLiquidationFee(D_coll)).add(addedColl.mul(toBN(3))))

    // E opens with another 1998 SEI
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected SEI rewards: 
     Carol = 1992.01/1998 * 1998*0.995 = 1982.04995 SEI
     Alice = 2.995/1998 * 1998*0.995 = 2.980025 SEI
     Bob = 2.995/1998 * 1998*0.995 = 2.980025 SEI

    therefore, expected total collateral:

    Carol = 1992.01 + 1982.04995 = 3974.05995
    Alice = 2.995 + 2.980025 = 5.975025 SEI
    Bob = 2.995 + 2.980025 = 5.975025 SEI
    ...
    */
    ...
    //Expect 3986.01 SEI in system now
    const entireSystemColl_3 = (await activePool.getSEI()).add(await defaultPool.getSEI())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    //Bob withdraws 0.5 SEI from his trove
    ...
    // Expect Bob now holds all Ether and SAIDebt in the system: 2.5 Ether and 300 SAI
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 SEI, 55 SAI
     L2: (0.9975/2.495)*0.995 = 0.3978 SEI , 110*(0.9975/2.495)= 43.98 SAIDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 SEI
    debt: (110 + 55 + 43.98 = 208.98 SAIDebt 

     Alice rewards:
    L1 0.4975, 55 SAI
    L2 (1.4975/2.495)*0.995 = 0.5972 SEI, 110*(1.4975/2.495) = 66.022 SAIDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 SEI
    debt: (50 + 55 + 66.022) = 171.022 SAI Debt

    totalColl: 3.49 SEI
    totalDebt 380 SAI (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingSEIReward(bob)))
      .toString()
    ...
    const entireSystemColl = (await activePool.getSEI()).add(await defaultPool.getSEI())
    ...
    const entireSystemDebt = (await activePool.getSAIDebt()).add(await defaultPool.getSAIDebt())
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    //Expect 1995 SEI in system now
    const entireSystemColl_1 = (await activePool.getSEI()).add(await defaultPool.getSEI())
    ...
    /* Expected SEI rewards: 
     Carol = 1990.01/1994 * 1994*0.995 = 1980.05995 SEI
     Alice = 1.995/1994 * 1994*0.995 = 1.985025 SEI
     Bob = 1.995/1994 * 1994*0.995 = 1.985025 SEI
    ...
    */
    ...
    //Expect 3978.03 SEI in system now
    const entireSystemColl_3 = (await activePool.getSEI()).add(await defaultPool.getSEI())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    /* Alice, Bob, Carol each withdraw 0.5 SEI to their troves, 
    ...
    //Expect 1993.5 SEI in system now
    const entireSystemColl_2 = (await activePool.getSEI()).add(await defaultPool.getSEI())
    ...
    // E opens with another 1993.5 SEI
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected SEI rewards: 
     Carol = 1990.51/1993.5 * 1993.5*0.995 = 1980.55745 SEI
     Alice = 1.495/1993.5 * 1993.5*0.995 = 1.487525 SEI
     Bob = 1.495/1993.5 * 1993.5*0.995 = 1.487525 SEI

    therefore, expected total collateral:

    Carol = 1990.51 + 1980.55745 = 3971.06745
    Alice = 1.495 + 1.487525 = 2.982525 SEI
    Bob = 1.495 + 1.487525 = 2.982525 SEI
    ...
    */
    ...
    //Expect 3977.0325 SEI in system now
    const entireSystemColl_3 = (await activePool.getSEI()).add(await defaultPool.getSEI())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    // A, B, C open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingSEIReward(bob), B_pendingRewardsAfterL1), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingSEIReward(carol), C_pendingRewardsAfterL1), 1000000)
    ...
    //Carol  withdraws 1 SEI from her trove
    const C_withdrawnColl = toBN(dec(1, 'ether'))
    await borrowerOperations.withdrawColl(C_withdrawnColl, carol, carol, { from: carol })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingSEIReward(carol), C_pendingRewardsAfterL2), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingSEIReward(dennis), D_pendingRewardsAfterL2), 1000000)
    ...
    // E and F open troves
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(110, 18), extraParams: { from: freddy } })
    ...
    const carol_pendingSEIReward = (await troveManager.getPendingSEIReward(carol)).toString()

    const dennis_rawColl = (await troveManager.Troves(dennis))[1].toString()
    const dennis_pendingSEIReward = (await troveManager.getPendingSEIReward(dennis)).toString()

    const erin_rawColl = (await troveManager.Troves(erin))[1].toString()
    const erin_pendingSEIReward = (await troveManager.getPendingSEIReward(erin)).toString()
    ...
    // Check pending SEI rewards of C, D, E
    assert.isAtMost(th.getDifference(carol_pendingSEIReward, C_collAfterL3.sub(C_collAfterL1)), 1000000)
    assert.isAtMost(th.getDifference(dennis_pendingSEIReward, D_collAfterL3.sub(D_collAfterL2)), 1000000)
    assert.isAtMost(th.getDifference(erin_pendingSEIReward, E_collAfterL3.sub(E_coll)), 1000000)

    // Check systemic collateral
    const activeColl = (await activePool.getSEI()).toString()
    const defaultColl = (await defaultPool.getSEI()).toString()
    ...
    // check SAI gas compensation
    assert.equal((await saiToken.balanceOf(owner)).toString(), dec(600, 18))
  })
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    // D opens trove: 0.035 SEI
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ extraSAIAmount: dec(100, 18), extraParams: { from: dennis, value: toBN(dec(35, 15)) } })

    // Bob adds 11.33909 SEI to his trove
    const B_addedColl = toBN('11339090000000000000')
    await borrowerOperations.addColl(bob, bob, { from: bob, value: B_addedColl })

    // Carol withdraws 15 SEI from her trove
    ...
    const D_pendingRewardsAfterL2 = D_coll.mul(th.applyLiquidationFee(B_collAfterL1)).div(C_collAfterL1.add(D_coll))
    const C_collAfterL2 = C_collAfterL1.add(C_pendingRewardsAfterL2)
    assert.isAtMost(th.getDifference(await troveManager.getPendingSEIReward(carol), C_pendingRewardsAfterL2), 10000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingSEIReward(dennis), D_pendingRewardsAfterL2), 10000000)
    ...
    F: 0.0007 SEI
    */
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ extraSAIAmount: dec(100, 18), extraParams: { from: erin, value: toBN(dec(1, 22)) } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ extraSAIAmount: dec(100, 18), extraParams: { from: freddy, value: toBN('700000000000000') } })
