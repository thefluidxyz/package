...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  //   // price drops to 1SEI:150SAI, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  //   //  Alice and Bob withdraw such that the TCR is ~150%
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  //   // --- TEST ---
  //   // price drops to 1SEI:100SAI, reducing TCR below 150%, and all Troves below 100% ICR
  //   await priceFeed.setPrice('100000000000000000000')
  ...
  //   //  Alice and Bob withdraw such that the TCR is ~150%
  //   await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   await openTrove({ ICR: toBN(dec(150, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraSAIAmount: spDeposit, extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  //   // --- SETUP ---
  //   //  Bob withdraws up to 2000 SAI of debt, bringing his ICR to 210%
  //   const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  //   // --- TEST ---
  //   // price drops to 1SEI:100SAI, reducing TCR to 117%
  //   await priceFeed.setPrice('100000000000000000000')
  ...
  //   //  Bob withdraws up to 20000 SAI of debt, bringing his ICR to 210%
  //   const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraSAIAmount: dec(20000, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  //   // Alice deposits 390SAI to the Stability Pool
  //   await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })
  ...
  //   // check pool SAI before liquidation
  //   const stabilityPoolSAI_Before = (await stabilityPool.getTotalSAIDeposits()).toString()
  //   assert.equal(stabilityPoolSAI_Before, '390000000000000000000')
  ...
  //   /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 SAI.
    
  //   With 390 SAI in the StabilityPool, 390 SAI should be offset with the pool, leaving 0 in the pool.
  
  //   Stability Pool rewards for alice should be:
  //   SAILoss: 390SAI
  //   SEIGain: (390 / 2000) * 21*0.995 = 4.074525 ether

  //   After offsetting 390 SAI and 4.074525 ether, the remainders - 1610 SAI and 16.820475 ether - should be redistributed to all active Troves.
  //  */
  //   // Liquidate Bob
  //   await troveManager.liquidate(bob, { from: owner })

  //   const aliceDeposit = await stabilityPool.getCompoundedSAIDeposit(alice)
  //   const aliceSEIGain = await stabilityPool.getDepositorSEIGain(alice)
  //   const aliceExpectedSEIGain = spDeposit.mul(th.applyLiquidationFee(B_coll)).div(B_totalDebt)

  //   assert.equal(aliceDeposit.toString(), 0)
  //   assert.equal(aliceSEIGain.toString(), aliceExpectedSEIGain)

  //   /* Now, check redistribution to active Troves. Remainders of 1610 SAI and 16.82 ether are distributed.
  ...
  //   L_SAIDebt = 1610 / 6 = 268.333 SAI
  //   L_SEI = 16.820475 /6 =  2.8034125 ether
  //   */
  //   const L_SAIDebt = (await troveManager.L_SAIDebt()).toString()
  //   const L_SEI = (await troveManager.L_SEI()).toString()

  //   assert.isAtMost(th.getDifference(L_SAIDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
  //   assert.isAtMost(th.getDifference(L_SEI, th.applyLiquidationFee(B_coll.sub(B_coll.mul(spDeposit).div(B_totalDebt)).mul(mv._1e18BN).div(A_coll.add(D_coll)))), 100)
  // })
  ...
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  //   assert.equal(L_SAIDebt, '0')
  //   assert.equal(L_SEI, '0')
  ...
  // // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool SAI is GREATER THAN liquidated debt ---

  // it("liquidate(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: offsets the trove entirely with the pool", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: B_totalDebt, extraParams: { from: alice } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: dennis } })

  //   // Alice deposits SAI in the Stability Pool
  ...
  //   /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 SAI, Alice sole depositor.
  //   As liquidated debt (250 SAI) was completely offset

  //   Alice's expected compounded deposit: (1490 - 250) = 1240SAI
  //   Alice's expected SEI gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
  //   */
  //   const aliceExpectedDeposit = await stabilityPool.getCompoundedSAIDeposit(alice)
  //   const aliceExpectedSEIGain = await stabilityPool.getDepositorSEIGain(alice)

  //   assert.isAtMost(th.getDifference(aliceExpectedDeposit.toString(), spDeposit.sub(B_totalDebt)), 2000)
  //   assert.isAtMost(th.getDifference(aliceExpectedSEIGain, th.applyLiquidationFee(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price))), 3000)
  ...
  // it("liquidate(), with ICR% = 110 < TCR, and StabilityPool SAI > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 220%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: B_totalDebt, extraParams: { from: alice } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  // it("liquidate(), with  110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: removes stake and updates totalStakes", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  // it("liquidate(), with  110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: updates system snapshots", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  // it("liquidate(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: closes the Trove", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  // it("liquidate(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: can liquidate troves out of order", async () => {
  //   // taking out 1000 SAI, CR of 200%
  ...
  //   await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: totalLiquidatedDebt, extraParams: { from: whale } })
  ...
  // SAI is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  // it("liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: Trove remains active", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(1500, 18), extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: dennis } })

  //   // Alice deposits 1490 SAI in the Stability Pool
  ...
  //   /* Since the pool only contains 100 SAI, and Bob's pre-liquidation debt was 250 SAI,
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: Trove remains in TroveOwners array", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   // Alice deposits 100 SAI in the Stability Pool
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: nothing happens", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   /*  Since Bob's debt (250 SAI) is larger than all SAI in the Stability Pool, Liquidation won’t happen
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: updates system shapshots", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: causes correct Pool offset and SEI gain, and doesn't redistribute to active troves", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   assert.equal(aliceExpectedSEIGain.toString(), '0')
  ...
  //   const L_SAIDebt_After = (await troveManager.L_SAIDebt()).toString()
  //   const L_SEI_After = (await troveManager.L_SEI()).toString()

  //   assert.equal(L_SAIDebt_After, '0')
  //   assert.equal(L_SEI_After, '0')
  // })

  // it("liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: ICR of non liquidated trove does not change", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   // Carol withdraws up to debt of 240 SAI, -> ICR of 250%.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(1500, 18), extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: dec(2000, 18), extraParams: { from: dennis } })
  //   const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraSAIAmount: dec(240, 18), extraParams: { from: carol } })
  ...
  //   //Check SP SAI has been completely emptied
  //   assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), dec(100, 18))
  ...
  //   await saiToken.transfer(bob, dec(100, 18), { from: alice })
  ...
  //   // Alice provides another 50 SAI to pool
  ...
  //   //Check SP SAI has been completely emptied
  //   assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), dec(150, 18))
  ...
  // it("liquidate() with ICR > 110%, and StabilityPool SAI < liquidated debt: total liquidated coll and debt is correct", async () => {
  //   // Whale provides 50 SAI to the SP
  //   await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  //   // Alice creates a single trove with 0.62 SEI and a debt of 62 SAI, and provides 10 SAI to SP
  ...
  //   /*  Since there is 0 SAI in the stability Pool, A, with ICR >110%, should stay active.
  ...
  //   await openTrove({ ICR: toBN(dec(220, 16)), extraSAIAmount: spDeposit, extraParams: { from: bob } })

  //   // Bob sends tokens to Dennis, who has no trove
  //   await saiToken.transfer(dennis, spDeposit, { from: bob })

  //   //Dennis provides 200 SAI to SP
  ...
  //   assert.isAtMost(th.getDifference(dennis_SEIGain_Before, th.applyLiquidationFee(C_coll)), 1000)
  ...
  //   await openTrove({ ICR: toBN(dec(220, 16)), extraSAIAmount: dec(1000, 18), extraParams: { from: whale } })

  //   const { saiAmount: A_saiAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(300, 18), extraParams: { from: alice } })
  //   const { saiAmount: B_saiAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(200, 18), extraParams: { from: bob } })
  //   const { saiAmount: C_saiAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: carol } })
  ...
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 SAI of debt, resulting in ICRs of 266%.
  //   // Bob withdraws up to 480 SAI of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: dec(480, 18), extraParams: { from: bob } })
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: B_totalDebt, extraParams: { from: alice } })
  ...
  //   // Bob re-opens the trove, price 200, total debt 80 SAI, ICR = 120% (lowest one)
  //   // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 SEI
  //   await priceFeed.setPrice('200000000000000000000')
  //   const { collateral: B_coll_2, netDebt: B_netDebt_2 } = await openTrove({ ICR: toBN(dec(150, 16)), extraSAIAmount: dec(480, 18), extraParams: { from: bob, value: bob_remainingCollateral } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: B_netDebt_2, extraParams: { from: dennis } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 SAI of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraSAIAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 SAI of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: B_netDebt, extraParams: { from: dennis } })

    // --- TEST ---
    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Dennis redeems 40, so Bob has a surplus of (200 * 1 - 40) / 200 = 0.8 SEI
    ...
    // Bob re-opens the trove, price 200, total debt 250 SAI, ICR = 240% (lowest one)
    ...
    // Alice deposits SAI in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraSAIAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some SAI from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraSAIAmount: dec(5000, 18), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits SAI to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1SEI:90SAI, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1SEI:85SAI, reducing TCR below 150%
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraSAIAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraSAIAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraSAIAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraSAIAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
    ...
    // Check Stability Pool has 500 SAI
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraSAIAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraSAIAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraSAIAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraSAIAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraSAIAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 SAI in Stability Pool
    assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds SAI to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    await openTrove({ ICR: toBN(dec(285, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    ...
    // Confirm SAI in Stability Pool
    assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), spDeposit.toString())
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(await troveManager.getPendingSEIReward(alice))
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(await troveManager.getPendingSAIDebtReward(alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { saiAmount: saiAmountD } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: dennis } })
    const { saiAmount: saiAmountE } = await openTrove({ ICR: toBN(dec(133, 16)), extraParams: { from: erin } })
    const { saiAmount: saiAmountF } = await openTrove({ ICR: toBN(dec(111, 16)), extraParams: { from: freddy } })

    // Check list size is 4
    assert.equal((await sortedTroves.getSize()).toString(), '4')

    // Check token balances before
    assert.equal((await saiToken.balanceOf(dennis)).toString(), saiAmountD)
    assert.equal((await saiToken.balanceOf(erin)).toString(), saiAmountE)
    assert.equal((await saiToken.balanceOf(freddy)).toString(), saiAmountF)
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and SEI gain", async () => {
    // Whale provides SAI to the SP
    const { saiAmount: W_saiAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_saiAmount, ZERO_ADDRESS, { from: whale })

    const { saiAmount: A_saiAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraSAIAmount: dec(40, 18), extraParams: { from: alice } })
    const { saiAmount: B_saiAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(240, 18), extraParams: { from: bob } })
    ...
    await stabilityPool.provideToSP(A_saiAmount, ZERO_ADDRESS, { from: alice })
    await stabilityPool.provideToSP(B_saiAmount, ZERO_ADDRESS, { from: bob })

    const totalDeposit = W_saiAmount.add(A_saiAmount).add(B_saiAmount)
    ...
    // Check SAI in Pool
    assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), totalDeposit)
    ...
    Total SAI in Pool: 680 SAI

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 SAI
    Total liquidated SEI = 1 + 3 + 1 = 5 SEI

    Whale SAI Loss: 500 * (400/680) = 294.12 SAI
    Alice SAI Loss:  500 *(40/680) = 29.41 SAI
    Bob SAI Loss: 500 * (240/680) = 176.47 SAI

    Whale remaining deposit: (400 - 294.12) = 105.88 SAI
    Alice remaining deposit: (40 - 29.41) = 10.59 SAI
    Bob remaining deposit: (240 - 176.47) = 63.53 SAI

    Whale SEI Gain: 5*0.995 * (400/680) = 2.93 SEI
    Alice SEI Gain: 5*0.995 *(40/680) = 0.293 SEI
    Bob SEI Gain: 5*0.995 * (240/680) = 1.76 SEI

    Total remaining deposits: 180 SAI
    Total SEI gain: 5*0.995 SEI */
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedSAIDeposit(whale)).toString()
    ...
    const whale_SEIGain = (await stabilityPool.getDepositorSEIGain(whale)).toString()
    const alice_SEIGain = (await stabilityPool.getDepositorSEIGain(alice)).toString()
    const bob_SEIGain = (await stabilityPool.getDepositorSEIGain(bob)).toString()

    const liquidatedDebt = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    const liquidatedColl = A_coll.add(B_coll).add(C_coll)
    assert.isAtMost(th.getDifference(whale_Deposit_After, W_saiAmount.sub(liquidatedDebt.mul(W_saiAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(alice_Deposit_After, A_saiAmount.sub(liquidatedDebt.mul(A_saiAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(bob_Deposit_After, B_saiAmount.sub(liquidatedDebt.mul(B_saiAmount).div(totalDeposit))), 100000)

    assert.isAtMost(th.getDifference(whale_SEIGain, th.applyLiquidationFee(liquidatedColl).mul(W_saiAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(alice_SEIGain, th.applyLiquidationFee(liquidatedColl).mul(A_saiAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(bob_SEIGain, th.applyLiquidationFee(liquidatedColl).mul(B_saiAmount).div(totalDeposit)), 2000)
    ...
    assert.isAtMost(th.getDifference(total_SAIinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_SEIinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or SEI gain", async () => {
    // Whale provides 400 SAI to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraSAIAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check SAI and SEI in Pool  before
    ...
    assert.equal(SAIinSP_Before, dec(800, 18))
    assert.equal(SEIinSP_Before, '0')
    ...
    // Check SAI and SEI in Pool after
    ...
    const whale_SEIGain_After = (await stabilityPool.getDepositorSEIGain(whale)).toString()
    ...
    assert.equal(whale_SEIGain_After, '0')
    assert.equal(alice_SEIGain_After, '0')
    assert.equal(bob_SEIGain_After, '0')
  })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    That leaves 50 SAI in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides SAI to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 SAI in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 SAI in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 SAI and system coll 2.3 SEI
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides SAI to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 SAI in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10)

    const [liquidatedDebt, liquidatedColl, collGasComp, saiGasComp] = th.getEmittedLiquidationValues(liquidationTx)
    ...
    assert.equal(saiGasComp.toString(), dec(400, 18))
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraSAIAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraSAIAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraSAIAmount: A_totalDebt, extraParams: { from: whale } })

    // Alice deposits SAI to Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })

    // to compensate borrowing fee
    await saiToken.transfer(alice, A_totalDebt, { from: whale })
    // Alice closes trove
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 SAI and system coll by 2 SEI
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraSAIAmount: dec(500, 18), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraSAIAmount: dec(500, 18), extraParams: { from: freddy } })

    // Whale provides 1000 SAI to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(await troveManager.getPendingSAIDebtReward(dennis))
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(await troveManager.getPendingSAIDebtReward(bob))
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(await troveManager.getPendingSAIDebtReward(carol))

    const dennisColl_After = (await troveManager.Troves(dennis))[1].add(await troveManager.getPendingSEIReward(dennis))  
    const bobColl_After = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingSEIReward(bob))
    const carolColl_After = (await troveManager.Troves(carol))[1].add(await troveManager.getPendingSEIReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 SAI
    ...
    // Expected Stability Pool to fully absorb B (92 SAI + 10 virtual debt), 
    // but not E as there are not enough funds in Stability Pool
    
    const stabilityBefore = await stabilityPool.getTotalSAIDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalSAIDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 SAI to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
