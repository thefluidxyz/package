...
contract('All Fluid functions with onlyOwner modifier', async accounts => {
  ...
  describe('CommunityIssuance', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      const params = [floToken.address, stabilityPool.address]
  ...
  describe('FLOStaking', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      await testSetAddresses(floStaking, 5)
    })
  })

  describe('LockupContractFactory', async accounts => {
    it("setFLOAddress(): reverts when called by non-owner, with wrong address, or twice", async () => {
      await th.assertRevert(lockupContractFactory.setFLOTokenAddress(floToken.address, { from: alice }))

      const params = [floToken.address]

      // Attempt to use zero address
      await testZeroAddress(lockupContractFactory, params, 'setFLOTokenAddress')
      // Attempt to use non contract
      await testNonContractAddress(lockupContractFactory, params, 'setFLOTokenAddress')

      // Owner can successfully set any address
      const txOwner = await lockupContractFactory.setFLOTokenAddress(floToken.address, { from: owner })

      assert.isTrue(txOwner.receipt.status)
      // fails if called twice
      await th.assertRevert(lockupContractFactory.setFLOTokenAddress(floToken.address, { from: owner }))
    })
  })
})
