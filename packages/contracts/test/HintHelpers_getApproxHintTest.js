...
contract('HintHelpers', async accounts => {
  ...
  /* Open a Trove for each account. SAI debt is 200 SAI each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

 // Open Troves in parallel, then withdraw SAI in parallel
 const makeTrovesInParallel = async (accounts, n) => {
  ...
  const withdrawSAIpromises = activeAccounts.map(account => withdrawSAIfromTrove(account))
  await Promise.all(withdrawSAIpromises)
 ...
 const withdrawSAIfromTrove = async (account) => {
  await borrowerOperations.withdrawSAI(th._100pct, '100000000000000000000', account, account, { from: account })
 }

 // Sequentially add coll and withdraw SAI, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extraSAIAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
