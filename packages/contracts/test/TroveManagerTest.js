...
contract('TroveManager', async accounts => {
  ...
  const getOpenTroveTotalDebt = async (saiAmount) => th.getOpenTroveTotalDebt(contracts, saiAmount)
  ...
  const withdrawSAI = async (params) => th.withdrawSAI(contracts, params)
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    // Alice increases debt to 180 SAI, lowering her ICR to 1.11
    const A_SAIWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawSAI({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1SEI:100SAI, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool SEI and SAIDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool SEI and SAI debt before
    const activePool_SEI_Before = (await activePool.getSEI()).toString()
    const activePool_RawEther_Before = (await web3.eth.getBalance(activePool.address)).toString()
    const activePool_SAIDebt_Before = (await activePool.getSAIDebt()).toString()

    assert.equal(activePool_SEI_Before, A_collateral.add(B_collateral))
    assert.equal(activePool_RawEther_Before, A_collateral.add(B_collateral))
    th.assertIsApproximatelyEqual(activePool_SAIDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1SEI:100SAI, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his sei and SAI, 
    leaving Alice’s sei and SAI debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool SEI and SAI debt 
    const activePool_SEI_After = (await activePool.getSEI()).toString()
    const activePool_RawEther_After = (await web3.eth.getBalance(activePool.address)).toString()
    const activePool_SAIDebt_After = (await activePool.getSAIDebt()).toString()

    assert.equal(activePool_SEI_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePool_SAIDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool SEI and SAI debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(21, 17)), extraParams: { from: bob } })
    ...
    // check DefaultPool SEI and SAI debt before
    const defaultPool_SEI_Before = (await defaultPool.getSEI())
    const defaultPool_RawEther_Before = (await web3.eth.getBalance(defaultPool.address)).toString()
    const defaultPool_SAIDebt_Before = (await defaultPool.getSAIDebt()).toString()

    assert.equal(defaultPool_SEI_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_SAIDebt_Before, '0')
    ...
    // check after
    const defaultPool_SEI_After = (await defaultPool.getSEI()).toString()
    const defaultPool_RawEther_After = (await web3.eth.getBalance(defaultPool.address)).toString()
    const defaultPool_SAIDebt_After = (await defaultPool.getSAIDebt()).toString()

    const defaultPool_SEI = th.applyLiquidationFee(B_collateral)
    assert.equal(defaultPool_SEI_After, defaultPool_SEI)
    assert.equal(defaultPool_RawEther_After, defaultPool_SEI)
    th.assertIsApproximatelyEqual(defaultPool_SAIDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and SAI should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });

    /* check snapshots after. Total stakes should be equal to the  remaining stake then the system: 
    10 ether, Alice's stake.
     
    Total collateral should be equal to Alice's collateral plus her pending SEI reward (Bob’s collaterale*0.995 ether), earned
  ...
  it("liquidate(): updates the L_SEI and L_SAIDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1SEI:100SAI, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's ether*0.995 and SAI should be added to the DefaultPool.
    const L_SEI_AfterCarolLiquidated = await troveManager.L_SEI()
    const L_SAIDebt_AfterCarolLiquidated = await troveManager.L_SAIDebt()

    const L_SEI_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_SAIDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_SEI_AfterCarolLiquidated, L_SEI_expected_1), 100)
    assert.isAtMost(th.getDifference(L_SAIDebt_AfterCarolLiquidated, L_SAIDebt_expected_1), 100)

    // Bob now withdraws SAI, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawSAI({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1SEI:50SAI, reducing Bob's ICR below MCR
   ...
   L_SEI = (0.995 / 20) + (10.4975*0.995  / 10) = 1.09425125 SEI
   L_SAIDebt = (180 / 20) + (890 / 10) = 98 SAI */
    const L_SEI_AfterBobLiquidated = await troveManager.L_SEI()
    const L_SAIDebt_AfterBobLiquidated = await troveManager.L_SAIDebt()

    const L_SEI_expected_2 = L_SEI_expected_1.add(th.applyLiquidationFee(B_collateral.add(B_collateral.mul(L_SEI_expected_1).div(mv._1e18BN))).mul(mv._1e18BN).div(A_collateral))
    const L_SAIDebt_expected_2 = L_SAIDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_SAIDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_SEI_AfterBobLiquidated, L_SEI_expected_2), 100)
    assert.isAtMost(th.getDifference(L_SAIDebt_AfterBobLiquidated, L_SAIDebt_expected_2), 100)
  })

  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: bob, value: dec(100, 'ether') } })

    // Alice creates a single trove with 0.7 SEI and a debt of 70 SAI, and provides 10 SAI to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 SAI to SP
    await stabilityPool.provideToSP(dec(10, 18), ZERO_ADDRESS, { from: alice })

    // Set SEI:USD price to 105
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides SAI to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or SEI gain when called on an SP depositor's address that has no trove", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraSAIAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await saiToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides SAI to SP
    ...
    assert.isFalse(await sortedTroves.contains(carol))
    // Check Dennis' SP deposit has absorbed Carol's debt, and he has received her liquidated SEI
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedSAIDeposit(dennis)).toString()
    const dennis_SEIGain_Before = (await stabilityPool.getDepositorSEIGain(dennis)).toString()
    assert.isAtMost(th.getDifference(dennis_Deposit_Before, spDeposit.sub(liquidatedDebt)), 1000000)
    assert.isAtMost(th.getDifference(dennis_SEIGain_Before, liquidatedColl), 1000)
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedSAIDeposit(dennis)).toString()
    const dennis_SEIGain_After = (await stabilityPool.getDepositorSEIGain(dennis)).toString()
    assert.equal(dennis_Deposit_Before, dennis_Deposit_After)
    assert.equal(dennis_SEIGain_Before, dennis_SEIGain_After)
  })

  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or SEI gain", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraSAIAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides SAI to SP
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated SEI
    ...
    assert.isAtMost(th.getDifference(bob_SEIGain_Before, liquidatedColl), 1000)
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and SEI gain", async () => {
    const A_spDeposit = toBN(dec(3, 24))
    const B_spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraSAIAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraSAIAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(bob_SEIGain_Before, th.applyLiquidationFee(C_collateral)), 1000)

    // Alice provides SAI to SP
    ...
    /* Alice's SAI Loss = (300 / 400) * 200 = 150 SAI
       Alice's SEI gain = (300 / 400) * 2*0.995 = 1.4925 SEI

       Bob's SAILoss = (100 / 400) * 200 = 50 SAI
       Bob's SEI gain = (100 / 400) * 2*0.995 = 0.4975 SEI

     Check Bob' SP deposit has been reduced to 50 SAI, and his SEI gain has increased to 1.5 SEI. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedSAIDeposit(alice)).toString()
    ...
    assert.isAtMost(th.getDifference(alice_SEIGain_After, th.applyLiquidationFee(B_collateral).mul(A_spDeposit).div(totalDeposits)), 1000000)

    const bob_Deposit_After = await stabilityPool.getCompoundedSAIDeposit(bob)
    const bob_SEIGain_After = await stabilityPool.getDepositorSEIGain(bob)
    ...
    assert.isAtMost(th.getDifference(bob_SEIGain_After, bob_SEIGain_Before.add(th.applyLiquidationFee(B_collateral).mul(bob_Deposit_Before).div(totalDeposits))), 1000000)
  })

  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { saiAmount: A_saiAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { saiAmount: B_saiAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { saiAmount: C_saiAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeSAIDebt_0 = await activePool.getSAIDebt()
    const defaultSAIDebt_0 = await defaultPool.getSAIDebt()

    await troveManager.liquidate(alice)
    const activeSAIDebt_A = await activePool.getSAIDebt()
    const defaultSAIDebt_A = await defaultPool.getSAIDebt()

    await troveManager.liquidate(bob)
    const activeSAIDebt_B = await activePool.getSAIDebt()
    const defaultSAIDebt_B = await defaultPool.getSAIDebt()
    ...
    assert.equal((await saiToken.balanceOf(alice)).toString(), A_saiAmount)
    assert.equal((await saiToken.balanceOf(bob)).toString(), B_saiAmount)
    assert.equal((await saiToken.balanceOf(carol)).toString(), C_saiAmount)
  })

  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Defaulter opens with 60 SAI, 0.6 SEI
    ...
    /* Liquidate defaulter. 30 SAI and 0.3 SEI is distributed between A, B and C.

    A receives (30 * 2/4) = 15 SAI, and (0.3*2/4) = 0.15 SEI
    B receives (30 * 1/4) = 7.5 SAI, and (0.3*1/4) = 0.075 SEI
    C receives (30 * 1/4) = 7.5 SAI, and (0.3*1/4) = 0.075 SEI
    */
    await troveManager.liquidate(defaulter_1)
  ...
  it("liquidate(): when SP > 0, triggers FLO reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalSAIDeposits(), dec(100, 18))

    const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1SEI:100SAI, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 SAI to the SP, but less than C's debt
    ...
    await borrowerOperations.repaySAI(dec(1, 18), D, D, {from: D})
    await borrowerOperations.repaySAI(dec(1, 18), E, E, {from: E})
    ...
    const pendingSEI_C = await troveManager.getPendingSEIReward(C)
    const pendingSAIDebt_C = await troveManager.getPendingSAIDebtReward(C)
    const defaultPoolSEI = await defaultPool.getSEI()
    const defaultPoolSAIDebt = await defaultPool.getSAIDebt()
    assert.isTrue(pendingSEI_C.lte(defaultPoolSEI))
    assert.isTrue(pendingSAIDebt_C.lte(defaultPoolSAIDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingSEI_C, defaultPoolSEI), 1000)
    assert.isAtMost(th.getDifference(pendingSAIDebt_C, defaultPoolSAIDebt), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1SEI:100SAI, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 SAI and 0.3 SEI is distributed uniformly between A, B and C. Each receive 10 SAI, 0.1 SEI
    await troveManager.liquidate(defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraSAIAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await saiToken.balanceOf(dennis)
    const E_balanceBefore = await saiToken.balanceOf(erin)
    const F_balanceBefore = await saiToken.balanceOf(flyn)
    ...
    assert.equal((await saiToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await saiToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await saiToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 SAI to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraSAIAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 SAI
    assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), '0')
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and SEI gain", async () => {
    // Whale provides 400 SAI to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraSAIAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 SAI in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalSAIDeposits()).toString(), totalDeposits)
    ...
    Total SAI in Pool: 800 SAI

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 SAI
    Total liquidated SEI = 1.1 + 3.1 + 1.1 = 5.3 SEI

    whale sai loss: 650 * (400/800) = 325 sai
    alice sai loss:  650 *(100/800) = 81.25 sai
    bob sai loss: 650 * (300/800) = 243.75 sai

    whale remaining deposit: (400 - 325) = 75 sai
    alice remaining deposit: (100 - 81.25) = 18.75 sai
    bob remaining deposit: (300 - 243.75) = 56.25 sai
    ...
    Total remaining deposits: 150 SAI
    Total SEI gain: 4.975 SEI */

    // Check remaining SAI Deposits and SEI gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedSAIDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedSAIDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedSAIDeposit(bob)

    const whale_SEIGain = await stabilityPool.getDepositorSEIGain(whale)
    const alice_SEIGain = await stabilityPool.getDepositorSEIGain(alice)
    const bob_SEIGain = await stabilityPool.getDepositorSEIGain(bob)
    ...
    assert.isAtMost(th.getDifference(whale_SEIGain, th.applyLiquidationFee(liquidatedColl).mul(whaleDeposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(alice_SEIGain, th.applyLiquidationFee(liquidatedColl).mul(A_deposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(bob_SEIGain, th.applyLiquidationFee(liquidatedColl).mul(B_deposit).div(totalDeposits)), 100000)

    // Check total remaining deposits and SEI gain in Stability Pool
    const total_SAIinSP = (await stabilityPool.getTotalSAIDeposits()).toString()
    const total_SEIinSP = (await stabilityPool.getSEI()).toString()

    assert.isAtMost(th.getDifference(total_SAIinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_SEIinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): when SP > 0, triggers FLO reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraSAIAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1SEI:100SAI, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraSAIAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalSAIDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await saiToken.transfer(carol, dec(100, 18), { from: whale })
  ...
  it("batchLiquidateTroves: when SP > 0, triggers FLO reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraSAIAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra SAI. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided SAI with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraSAIAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraSAIAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_SEIBalance_Before = toBN(await web3.eth.getBalance(dennis))

    const dennis_SAIBalance_Before = await saiToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 SAI
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    const SEIFee = th.getEmittedRedemptionValues(redemptionTx)[3]
    ...
    /* check that Dennis' redeemed 20 SAI has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) SAI debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(alice_debt_After, A_totalDebt.sub(partialRedemptionAmount))
    assert.equal(bob_debt_After, '0')
    assert.equal(carol_debt_After, '0')

    const dennis_SEIBalance_After = toBN(await web3.eth.getBalance(dennis))
    const receivedSEI = dennis_SEIBalance_After.sub(dennis_SEIBalance_Before)

    const expectedTotalSEIDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount SAI to SEI, at SEI:USD price 200
    const expectedReceivedSEI = expectedTotalSEIDrawn.sub(toBN(SEIFee)).sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)) // substract gas used for troveManager.redeemCollateral from expected received SEI
    
    // console.log("*********************************************************************************")
    // console.log("SEIFee: " + SEIFee)
    // console.log("dennis_SEIBalance_Before: " + dennis_SEIBalance_Before)
    // console.log("GAS_USED: " + th.gasUsed(redemptionTx))
    // console.log("dennis_SEIBalance_After: " + dennis_SEIBalance_After)
    // console.log("expectedTotalSEIDrawn: " + expectedTotalSEIDrawn)
    // console.log("recived  : " + receivedSEI)
    // console.log("expected : " + expectedReceivedSEI)
    // console.log("wanted :   " + expectedReceivedSEI.sub(toBN(GAS_PRICE)))
    // console.log("*********************************************************************************")
    th.assertIsApproximatelyEqual(expectedReceivedSEI, receivedSEI)

    const dennis_SAIBalance_After = (await saiToken.balanceOf(dennis)).toString()
    assert.equal(dennis_SAIBalance_After, dennis_SAIBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100SEI, 100 SAI), 20000%
    const { saiAmount: F_saiAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraSAIAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 SAI
    const flynBalance = await saiToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(flynBalance, F_saiAmount.sub(redemptionAmount))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 SAI
    const flynBalance = (await saiToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(flynBalance, F_saiAmount.sub(redemptionAmount))
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveSAIAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveSAIAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveSAIAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })

    // A and C send all their tokens to B
    await saiToken.transfer(B, await saiToken.balanceOf(A), {from: A})
    await saiToken.transfer(B, await saiToken.balanceOf(C), {from: C})
    ...
    // SAI redemption is 55000 US
    const SAIRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, SAIRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveSAIAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveSAIAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveSAIAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraSAIAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraSAIAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 SAI from Carol's Trove
    ...
    // Since Alice already redeemed 1 SAI from Carol's Trove, Dennis was  able to redeem:
    //  - 9 SAI from Carol's
    //  - 8 SAI from Bob's
    // for a total of 17 SAI.

    // Dennis calculated his hint for redeeming 2 SAI from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 SAI to fully complete his redemption of 20 SAI.
    // This would have required a different hint, therefore he ended up with a partial redemption.

    const dennis_SEIBalance_After = toBN(await web3.eth.getBalance(dennis))
    const receivedSEI = dennis_SEIBalance_After.sub(dennis_SEIBalance_Before)

    // Expect only 17 worth of SEI drawn
    const expectedTotalSEIDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted SAI converted to SEI, at SEI:USD price 200
    const expectedReceivedSEI = expectedTotalSEIDrawn.sub(SEIFee).sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)) // substract gas used for troveManager.redeemCollateral from expected received SEI

    th.assertIsApproximatelyEqual(expectedReceivedSEI, receivedSEI)

    const dennis_SAIBalance_After = (await saiToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_SAIBalance_After, dennis_SAIBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveSAIAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraSAIAmount: amount, extraParams: { from: bob } })

    await saiToken.transfer(carol, amount, { from: bob })
    ...
    const carol_SEIBalance_Before = toBN(await web3.eth.getBalance(carol))
    ...
    const carol_SEIBalance_After = toBN(await web3.eth.getBalance(carol))

    const expectedTotalSEIDrawn = toBN(amount).div(toBN(100)) // convert 100 SAI to SEI at SEI:USD price of 100
    const expectedReceivedSEI = expectedTotalSEIDrawn.sub(SEIFee)

    const receivedSEI = carol_SEIBalance_After.sub(carol_SEIBalance_Before)
    assert.isTrue(expectedReceivedSEI.eq(receivedSEI))

    const carol_SAIBalance_After = (await saiToken.balanceOf(carol)).toString()
    assert.equal(carol_SAIBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { saiAmount: B_saiAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraSAIAmount: A_debt, extraParams: { from: bob } })

    await saiToken.transfer(carol, B_saiAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraSAIAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraSAIAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500SAI to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(500, 18), extraParams: { from: alice } })
    await saiToken.transfer(erin, dec(500, 18), { from: alice })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)

    // Check total SAI supply
    ...
    // SAI redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedSAIRedemption = expectedTotalSupply.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSAIRedemption, lessThan5pct), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)  // artificially zero the baseRate
    
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSAIRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSAIRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSAIRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraSAIAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraSAIAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // SAI redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedSAIRedemption = expectedTotalSupply.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const SEIDrawn = attemptedSAIRedemption.mul(mv._1e18BN).div(price)
    const slightlyMoreThanFee = (await troveManager.getRedemptionFeeWithDecay(SEIDrawn))
    const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedSAIRedemption, slightlyMoreThanFee)
    assert.isTrue(tx1.receipt.status)

    await troveManager.setBaseRate(0)  // Artificially zero the baseRate
    
    // Attempt with maxFee = 5.5%
    const exactSameFee = (await troveManager.getRedemptionFeeWithDecay(SEIDrawn))
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedSAIRedemption, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedSAIRedemption, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedSAIRedemption, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedSAIRedemption, dec(1, 18))
    assert.isTrue(tx5.receipt.status)
  })

  it("redeemCollateral(): doesn't affect the Stability Pool deposits or SEI gain of redeemed-from troves", async () => {
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraSAIAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers SAI to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: redemptionAmount, extraParams: { from: alice } })
    await saiToken.transfer(erin, redemptionAmount, { from: alice })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedSAIDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedSAIDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedSAIDeposit(dennis)).toString()

    const bob_SEIGain_before = (await stabilityPool.getDepositorSEIGain(bob)).toString()
    const carol_SEIGain_before = (await stabilityPool.getDepositorSEIGain(carol)).toString()
    const dennis_SEIGain_before = (await stabilityPool.getDepositorSEIGain(dennis)).toString()

    // Check the remaining SAI and SEI in Stability Pool after liquidation is non-zero
    const SAIinSP = await stabilityPool.getTotalSAIDeposits()
    const SEIinSP = await stabilityPool.getSEI()
    assert.isTrue(SAIinSP.gte(mv._zeroBN))
    assert.isTrue(SEIinSP.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedSAIDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedSAIDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedSAIDeposit(dennis)).toString()

    const bob_SEIGain_after = (await stabilityPool.getDepositorSEIGain(bob)).toString()
    const carol_SEIGain_after = (await stabilityPool.getDepositorSEIGain(carol)).toString()
    const dennis_SEIGain_after = (await stabilityPool.getDepositorSEIGain(dennis)).toString()

    // Check B, C, D Stability Pool deposits and SEI gain have not been affected by redemptions from their troves
  ...
  it("redeemCollateral(): caller can redeem their entire SAIToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 SAI to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(400, 18), extraParams: { from: alice } })
    await saiToken.transfer(erin, dec(400, 18), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await saiToken.balanceOf(erin)
    assert.equal(erin_balance_before, dec(400, 18))

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraSAIAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    const activePool_coll_before = await activePool.getSEI()
    ...
    // Erin attempts to redeem 400 SAI
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 SAI
    const activePool_debt_after = await activePool.getSAIDebt()
    assert.equal(activePool_debt_before.sub(activePool_debt_after), dec(400, 18))

    /* Check ActivePool coll reduced by $400 worth of Ether: at SEI:USD price of $200, this should be 2 SEI.

    therefore remaining ActivePool SEI should be 198 */
    const activePool_coll_after = await activePool.getSEI()
    ...
    const erin_balance_after = (await saiToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's SAI token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    // Get active debt and coll before redemption
    const activePool_debt_before = await activePool.getSAIDebt()
    const activePool_coll_before = (await activePool.getSEI()).toString()
    ...
    try {
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's SAI token balance")
    }

    // Erin tries to redeem 401 SAI
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 SAI
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 SAI
  ...
  it("redeemCollateral(): value of issued SEI == face value of redeemed SAI (assuming 1 SAI has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 1000 SAI each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraSAIAmount: dec(4990, 18), extraParams: { from: alice } })
    await saiToken.transfer(erin, dec(1000, 18), { from: alice })
    await saiToken.transfer(flyn, dec(1000, 18), { from: alice })
    await saiToken.transfer(graham, dec(1000, 18), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraSAIAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraSAIAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraSAIAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_SAI = '120000000000000000000'
    const _373_SAI = '373000000000000000000'
    const _950_SAI = '950000000000000000000'

    // Check Ether in activePool
    const activeSEI_0 = await activePool.getSEI()
    assert.equal(activeSEI_0, totalColl.toString());

    let firstRedemptionHint
    let partialRedemptionHintNICR


    // Erin redeems 120 SAI
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_SAI, price, 0))
    ...
    /* 120 SAI redeemed.  Expect $120 worth of SEI removed. At SEI:USD price of $200, 
    SEI removed = (120/200) = 0.6 SEI
    Total active SEI = 280 - 0.6 = 279.4 SEI */

    const activeSEI_1 = await activePool.getSEI()
    assert.equal(activeSEI_1.toString(), activeSEI_0.sub(toBN(_120_SAI).mul(mv._1e18BN).div(price)));

    // Flyn redeems 373 SAI
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_SAI, price, 0))
    ...
    /* 373 SAI redeemed.  Expect $373 worth of SEI removed. At SEI:USD price of $200, 
    SEI removed = (373/200) = 1.865 SEI
    Total active SEI = 279.4 - 1.865 = 277.535 SEI */
    const activeSEI_2 = await activePool.getSEI()
    assert.equal(activeSEI_2.toString(), activeSEI_1.sub(toBN(_373_SAI).mul(mv._1e18BN).div(price)));

    // Graham redeems 950 SAI
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_SAI, price, 0))
    ...
    /* 950 SAI redeemed.  Expect $950 worth of SEI removed. At SEI:USD price of $200, 
    SEI removed = (950/200) = 4.75 SEI
    Total active SEI = 277.535 - 4.75 = 272.785 SEI */
    const activeSEI_3 = (await activePool.getSEI()).toString()
    assert.equal(activeSEI_3.toString(), activeSEI_2.sub(toBN(_950_SAI).mul(mv._1e18BN).div(price)));
  })
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint SAI to Bob
    await saiToken.unprotectedMint(bob, dec(100, 18))

    assert.equal((await saiToken.balanceOf(bob)), dec(100, 18))
    ...
    // Bob tries to redeem his illegally obtained SAI
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint SAI to Bob
    await saiToken.unprotectedMint(bob, '101000000000000000000')

    assert.equal((await saiToken.balanceOf(bob)), '101000000000000000000')

    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraSAIAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraSAIAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getSAIDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 SAI, from a system that has 100 SAI outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await saiToken.balanceOf(A)

    await th.redeemCollateral(A, contracts, dec(10, 18), GAS_PRICE)

    // Check A's balance has decreased by 10 SAI
    assert.equal(await saiToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await saiToken.balanceOf(A)
    const B_balanceBefore = await saiToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 SAI
    assert.equal(await saiToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 SAI
    assert.equal(A_balanceBefore.sub(await saiToken.balanceOf(A)), dec(10, 18))
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero SEIFee to FLO staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 FLO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await floToken.approve(floStaking.address, dec(1, 18), { from: multisig })
    await floStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check FLO Staking contract balance before is zero
    const floStakingBalance_Before = await web3.eth.getBalance(floStaking.address)
    assert.equal(floStakingBalance_Before, '0')
    ...
    // Check FLO Staking contract balance after is non-zero
    const floStakingBalance_After = toBN(await web3.eth.getBalance(floStaking.address))
    assert.isTrue(floStakingBalance_After.gt(toBN('0')))
  })

  it("redeemCollateral(): a redemption made at zero base increases the SEI-fees-per-FLO-staked in FLO Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 FLO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await floToken.approve(floStaking.address, dec(1, 18), { from: multisig })
    await floStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check FLO Staking SEI-fees-per-FLO-staked before is zero
    ...
    // Check FLO Staking SEI-fees-per-FLO-staked after is non-zero
    const F_SEI_After = await floStaking.F_SEI()
    assert.isTrue(F_SEI_After.gt('0'))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero SEIFee to FLO staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 FLO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await floToken.approve(floStaking.address, dec(1, 18), { from: multisig })
    await floStaking.stake(dec(1, 18), { from: multisig })
    ...
    const floStakingBalance_Before = toBN(await web3.eth.getBalance(floStaking.address))
    ...
    // check FLO Staking balance has increased
    assert.isTrue(floStakingBalance_After.gt(floStakingBalance_Before))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate increases SEI-per-FLO-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 FLO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await floToken.approve(floStaking.address, dec(1, 18), { from: multisig })
    await floStaking.stake(dec(1, 18), { from: multisig })
    ...
    // check FLO Staking balance has increased
    assert.isTrue(F_SEI_After.gt(F_SEI_Before))
  })

  it("redeemCollateral(): a redemption sends the SEI remainder (SEIDrawn - SEIFee) to the redeemer", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 FLO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await floToken.approve(floStaking.address, dec(1, 18), { from: multisig })
    await floStaking.stake(dec(1, 18), { from: multisig })
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total SAI supply
    const activeSAI = await activePool.getSAIDebt()
    const defaultSAI = await defaultPool.getSAIDebt()

    const totalSAISupply = activeSAI.add(defaultSAI)
    th.assertIsApproximatelyEqual(totalSAISupply, totalDebt)
    ...
    /*
    At SEI:USD price of 200:
    SEIDrawn = (9 / 200) = 0.045 SEI
    SEIfee = (0.005 + (1/2) *( 9/260)) * SEIDrawn = 0.00100384615385 SEI
    SEIRemainder = 0.045 - 0.001003... = 0.0439961538462
    */

    const A_balanceAfter = toBN(await web3.eth.getBalance(A))

    // check A's SEI balance has increased by 0.045 SEI 
    const price = await priceFeed.getPrice()
    const SEIDrawn = redemptionAmount.mul(mv._1e18BN).div(price)
    th.assertIsApproximatelyEqual(
      A_balanceAfter.sub(A_balanceBefore),
      SEIDrawn.sub(
        toBN(dec(5, 15)).add(redemptionAmount.mul(mv._1e18BN).div(totalDebt).div(toBN(2)))
          .mul(SEIDrawn).div(mv._1e18BN)
      ).sub(toBN(gasUsed * GAS_PRICE)), // substract gas used for troveManager.redeemCollateral from expected received SEI
      100000
    )
  })
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraSAIAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems 360 SAI.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    // whale redeems SAI.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, GAS_PRICE)
    ...
    /*
    At SEI:USD price of 200, with full redemptions from A, B, C:

    SEIDrawn from A = 100/200 = 0.5 SEI --> Surplus = (1-0.5) = 0.5
    SEIDrawn from B = 120/200 = 0.6 SEI --> Surplus = (1-0.6) = 0.4
    SEIDrawn from C = 130/200 = 0.65 SEI --> Surplus = (2-0.65) = 1.35
    */
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems SAI.  Expect this to fully redeem A, B, C, and partially redeem 15 SAI from D.
    ...
    /* Expect D to have lost 15 debt and (at SEI price of 200) 15/200 = 0.075 SEI. 
    So, expect remaining debt = (85 - 15) = 70, and remaining SEI = 1 - 15/200 = 0.925 remaining. */
    const price = await priceFeed.getPrice()
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's SEI surplus (collateral - SEI drawn) available for the trove owner to claim", async () => {
    const {
      A_netDebt, A_coll,
      B_netDebt, B_coll,
      C_netDebt, C_coll,
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's SEI surplus (collateral - SEI drawn) available for the trove owner after re-opening trove", async () => {
    const {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraSAIAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { saiAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraSAIAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(saiAmount, price, 0)
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(th._100pct, 0, saiAmount, true, alice, alice, { from: alice, value: saiAmount.mul(mv._1e18BN).div(price) })
    }
  ...
  it("getPendingSAIDebtReward(): Returns 0 if there is no pending SAIDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraSAIAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraSAIAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_SAIDebt = await troveManager.L_SAIDebt()
    assert.equal(current_L_SAIDebt, 0)

    const carolSnapshot_L_SAIDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_SAIDebt, 0)

    const carol_PendingSAIDebtReward = await troveManager.getPendingSAIDebtReward(carol)
    assert.equal(carol_PendingSAIDebtReward, 0)
  })

  it("getPendingSEIReward(): Returns 0 if there is no pending SEI reward", async () => {
    // make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraSAIAmount: dec(100, 18), extraParams: { from: defaulter_1 } })
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_SEI = await troveManager.L_SEI()
    assert.equal(current_L_SEI, 0)

    const carolSnapshot_L_SEI = (await troveManager.rewardSnapshots(carol))[0]
    assert.equal(carolSnapshot_L_SEI, 0)

    const carol_PendingSEIReward = await troveManager.getPendingSEIReward(carol)
    assert.equal(carol_PendingSEIReward, 0)
  })
  ...
  it("computeICR(): Returns 2^256-1 for SEI:USD = 100, coll = 1 SEI, debt = 100 SAI", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for SEI:USD = 100, coll = 200 SEI, debt = 30 SAI", async () => {
    const price = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for SEI:USD = 250, coll = 1350 SEI, debt = 127 SAI", async () => {
  ...
  it("computeICR(): returns correct ICR for SEI:USD = 100, coll = 1 SEI, debt = 54321 SAI", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraSAIAmount: B_totalDebt, extraParams: { from: A } })

    // to be able to repay:
    await saiToken.transfer(B, B_totalDebt, { from: A })
