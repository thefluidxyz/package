...
const logFLOBalanceAndError = (FLOBalance_A, expectedFLOBalance_A) => {
  console.log(
    `Expected final balance: ${expectedFLOBalance_A}, \n
    Actual final balance: ${FLOBalance_A}, \n
    Abs. error: ${expectedFLOBalance_A.sub(FLOBalance_A)}`
  )
}

const repeatedlyIssueFLO = async (stabilityPool, timeBetweenIssuances, duration) => {
  const startTimestamp = th.toBN(await th.getLatestBlockTimestamp(web3))
  let timePassed = 0

  // while current time < 1 month from deployment, issue FLO every minute
  while (timePassed < duration) {
    // console.log(`timePassed: ${timePassed}`)
    await th.fastForwardTime(timeBetweenIssuances, web3.currentProvider)
    await stabilityPool._unprotectedTriggerFLOIssuance()
...
contract('FLO community issuance arithmetic tests', async accounts => {
  let contracts
  ...
  it("getCumulativeIssuanceFraction(): fraction doesn't increase if less than a minute has passed", async () => {
   // progress time 1 week 
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
  ...
  it("Cumulative issuance fraction is 0.0000013 after a minute", async () => {
    // console.log(`supply cap: ${await communityIssuanceTester.FLOSupplyCap()}`)
  ...
  it("Total FLO tokens issued is 42.20 after a minute", async () => {
    ...
    // Issue FLO
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '42200713760820460000'

    const absError = th.toBN(expectedTotalFLOIssued).sub(totalFLOIssued)
    // console.log(
    //   `time since deployment: ${duration}, 
    //    totalFLOIssued: ${totalFLOIssued},  
    //    expectedTotalFLOIssued: ${expectedTotalFLOIssued},
    //    abs. error: ${absError}`
    // )

    assert.isAtMost(th.getDifference(totalFLOIssued, expectedTotalFLOIssued), 1000000000000000)
  })

  it("Total FLO tokens issued is 2,531.94 after an hour", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '2531944322115010000000'
  ...
  it("Total FLO tokens issued is 60,711.40 after a day", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '60711403150133240000000'
  ...
  it("Total FLO tokens issued is 422,568.60 after a week", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '422568600980110200000000'
  ...
  it("Total FLO tokens issued is 1,772,113.21 after a month", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '1772113218814930000000000'
  ...
  it("Total FLO tokens issued is 5,027,363.22 after 3 months", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '5027363224065180000000000'
  ...
  it("Total FLO tokens issued is 9,264,902.04 after 6 months", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '9264902042296516000000000'
  ...
  it("Total FLO tokens issued is 16,000,000 after a year", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '16000000000000000000000000'
  ...
  it("Total FLO tokens issued is 24,000,000 after 2 years", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '24000000000000000000000000'
  ...
  it("Total FLO tokens issued is 28,000,000 after 3 years", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '28000000000000000000000000'
  ...
  it("Total FLO tokens issued is 30,000,000 after 4 years", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '30000000000000000000000000'
  ...
  it("Total FLO tokens issued is 31,968,750 after 10 years", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '31968750000000000000000000'
  ...
  it("Total FLO tokens issued is 31,999,969.48 after 20 years", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '31999969482421880000000000'
  ...
  it("Total FLO tokens issued is 31,999,999.97 after 30 years", async () => {
    const initialIssuance = await communityIssuanceTester.totalFLOIssued()
    assert.equal(initialIssuance, 0)
    ...
    await communityIssuanceTester.unprotectedIssueFLO()
    const totalFLOIssued = await communityIssuanceTester.totalFLOIssued()
    const expectedTotalFLOIssued = '31999999970197680000000000'
  ...
  it.skip("Frequent token issuance: issuance event every year, for 30 years", async () => {
    ...
    assert.isTrue(await stabilityPool.isEligibleForFLO(alice))

    const timeBetweenIssuances = timeValues.SECONDS_IN_ONE_YEAR
    const duration = await getDuration(timeValues.SECONDS_IN_ONE_YEAR * 30)

    await repeatedlyIssueFLO(stabilityPool, timeBetweenIssuances, duration)

    // Depositor withdraws their deposit and accumulated FLO
    await stabilityPool.withdrawFromSP(dec(1, 18), { from: alice })

    const FLOBalance_A = await floToken.balanceOf(alice)
    const expectedFLOBalance_A = th.toBN('33333333302289200000000000')
    const diff = expectedFLOBalance_A.sub(FLOBalance_A)

    // logFLOBalanceAndError(FLOBalance_A, expectedFLOBalance_A)
  ...
  it.skip("Frequent token issuance: issuance event every minute, for 1 month", async () => {
    ...
    const FLOBalance_A = await floToken.balanceOf(alice)
    const expectedFLOBalance_A = th.toBN('1845951269598880000000000')
    const diff = expectedFLOBalance_A.sub(FLOBalance_A)
