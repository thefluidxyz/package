...
on the Fluid admin address's capabilities during the first year are found in:

test/launchSequenceTest/DuringLockupPeriodTest.js */

contract('Access Control: Fluid functions with the caller restricted to Fluid contract(s)', async accounts => {
  ...
  before(async () => {
    ...
    coreContracts = await deploymentHelper.deploySAITokenTester(coreContracts)
    ...
    for (account of accounts.slice(0, 10)) {
      await th.openTrove(coreContracts, { extraSAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: account } })
    }

    const expectedCISupplyCap = '32000000000000000000000000' // 32mil

    // Check CI has been properly funded
    const bal = await floToken.balanceOf(communityIssuance.address)
    assert.equal(bal, expectedCISupplyCap)
  })

  describe('BorrowerOperations', async accounts => { 
    it("moveSEIGainToTrove(): reverts when called by an account that is not StabilityPool", async () => {
      // Attempt call from alice
      try {
        const tx1= await borrowerOperations.moveSEIGainToTrove(bob, bob, bob, { from: bob })
      } catch (err) {
  ...
  describe('ActivePool', async accounts => {
    // sendSEI
    it("sendSEI(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
      // Attempt call from alice
      try {
        const txAlice = await activePool.sendSEI(alice, 100, { from: alice })
    ...
    // increaseSAI	
    it("increaseSAIDebt(): reverts when called by an account that is not BO nor TroveM", async () => {
      // Attempt call from alice
      try {
        const txAlice = await activePool.increaseSAIDebt(100, { from: alice })
    ...
    // decreaseSAI
    it("decreaseSAIDebt(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
      // Attempt call from alice
      try {
        const txAlice = await activePool.decreaseSAIDebt(100, { from: alice })
  ...
  describe('DefaultPool', async accounts => {
    // sendSEIToActivePool
    it("sendSEIToActivePool(): reverts when called by an account that is not TroveManager", async () => {
      // Attempt call from alice
      try {
        const txAlice = await defaultPool.sendSEIToActivePool(100, { from: alice })
    ...
    // increaseSAI	
    it("increaseSAIDebt(): reverts when called by an account that is not TroveManager", async () => {
      // Attempt call from alice
      try {
        const txAlice = await defaultPool.increaseSAIDebt(100, { from: alice })
    ...
    // decreaseSAI	
    it("decreaseSAI(): reverts when called by an account that is not TroveManager", async () => {
      // Attempt call from alice
      try {
        const txAlice = await defaultPool.decreaseSAIDebt(100, { from: alice })
  ...
  describe('SAIToken', async accounts => {

    //    mint
    it("mint(): reverts when called by an account that is not BorrowerOperations", async () => {
      // Attempt call from alice
      const txAlice = saiToken.mint(bob, 100, { from: alice })
    ...
    it("burn(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
      // Attempt call from alice
      try {
        const txAlice = await saiToken.burn(bob, 100, { from: alice })
    ...
    it("sendToPool(): reverts when called by an account that is not StabilityPool", async () => {
      // Attempt call from alice
      try {
        const txAlice = await saiToken.sendToPool(bob, activePool.address, 100, { from: alice })
    ...
    it("returnFromPool(): reverts when called by an account that is not TroveManager nor StabilityPool", async () => {
      // Attempt call from alice
      try {
        const txAlice = await saiToken.returnFromPool(activePool.address, bob, 100, { from: alice })
  ...
  describe('LockupContract', async accounts => {
    it("withdrawFLO(): reverts when caller is not beneficiary", async () => {
      // deploy new LC with Carol as beneficiary
      const unlockTime = (await floToken.getDeploymentStartTime()).add(toBN(timeValues.SECONDS_IN_ONE_YEAR))
      ...
      // FLO Multisig funds the LC
      await floToken.transfer(LC.address, dec(100, 18), { from: multisig })

      // Fast-forward one year, so that beneficiary can withdraw
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)

      // Bob attempts to withdraw FLO
      try {
        const txBob = await LC.withdrawFLO({ from: bob })
      ...
      const txCarol = await LC.withdrawFLO({ from: carol })
      assert.isTrue(txCarol.receipt.status)
    })
  })

  describe('FLOStaking', async accounts => {
    it("increaseF_SAI(): reverts when caller is not TroveManager", async () => {
      try {
        const txAlice = await floStaking.increaseF_SAI(dec(1, 18), { from: alice })
  ...
  describe('FLOToken', async accounts => {
    it("sendToFLOStaking(): reverts when caller is not the FLOSstaking", async () => {
      // Check multisig has some FLO
      assert.isTrue((await floToken.balanceOf(multisig)).gt(toBN('0')))

      // multisig tries to call it
      try {
        const tx = await floToken.sendToFLOStaking(multisig, 1, { from: multisig })
      } catch (err) {
        assert.include(err.message, "revert")
      }
      ...
      // Owner transfers 1 FLO to bob
      await floToken.transfer(bob, dec(1, 18), { from: multisig })
      assert.equal((await floToken.balanceOf(bob)), dec(1, 18))

      // Bob tries to call it
      try {
        const tx = await floToken.sendToFLOStaking(bob, dec(1, 18), { from: bob })
      } catch (err) {
        assert.include(err.message, "revert")
      }
    })
  ...
  describe('CommunityIssuance', async accounts => {
    it("sendFLO(): reverts when caller is not the StabilityPool", async () => {
      const tx1 = communityIssuance.sendFLO(alice, dec(100, 18), {from: alice})
      const tx2 = communityIssuance.sendFLO(bob, dec(100, 18), {from: alice})
      const tx3 = communityIssuance.sendFLO(stabilityPool.address, dec(100, 18), {from: alice})
    ...
    it("issueFLO(): reverts when caller is not the StabilityPool", async () => {
      const tx1 = communityIssuance.issueFLO({from: alice})
