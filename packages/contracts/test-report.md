...
# Test output
The following is the output of a complete test run, made on commit [`a1824dd8`](https://github.com/fluid/dev/tree/a1824dd88f4928b424cae372d59c4455d5c9a2c2), from April 16th, 2021.
...
$ hardhat test
  ...
  Contract: Access Control: Fluid functions with the caller restricted to Fluid contract(s)
    BorrowerOperations
    ...
    ActivePool
      ...
      ✓ increaseSAIDebt(): reverts when called by an account that is not BO nor TroveM
      ✓ decreaseSAIDebt(): reverts when called by an account that is not BO nor TroveM nor SP
    ...
    DefaultPool
      ...
      ✓ increaseSAIDebt(): reverts when called by an account that is not TroveManager
      ✓ decreaseSAI(): reverts when called by an account that is not TroveManager
    ...
    LQTYStaking
      ✓ increaseF_SAI(): reverts when caller is not TroveManager
    LQTYToken
  ...
  Contract: BorrowerOperations
    Without proxy
      ...
      ✓ addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_SAIDebt snapshots (736ms)
      ...
      ✓ withdrawColl(): applies pending rewards and updates user's L_ETH, L_SAIDebt snapshots (1205ms)
      ✓ withdrawSAI(): reverts when withdrawal would leave trove with ICR < MCR (516ms)
      ✓ withdrawSAI(): decays a non-zero base rate (1192ms)
      ✓ withdrawSAI(): reverts if max fee > 100% (668ms)
      ✓ withdrawSAI(): reverts if max fee < 0.5% in Normal mode (639ms)
      ✓ withdrawSAI(): reverts if fee exceeds max fee percentage (938ms)
      ✓ withdrawSAI(): succeeds when fee is less than max fee percentage (1272ms)
      ✓ withdrawSAI(): doesn't change base rate if it is already zero (1044ms)
      ✓ withdrawSAI(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation (899ms)
      ✓ withdrawSAI(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity (852ms)
      ✓ withdrawSAI(): borrowing at non-zero base rate sends SAI fee to LQTY staking contract (1004ms)
      ✓ withdrawSAI(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (923ms)
      ✓ withdrawSAI(): Borrowing at non-zero base rate increases the LQTY staking contract SAI fees-per-unit-staked (986ms)
      ✓ withdrawSAI(): Borrowing at non-zero base rate sends requested amount to the user (1562ms)
      ✓ withdrawSAI(): Borrowing at zero base rate changes SAI fees-per-unit-staked (839ms)
      ✓ withdrawSAI(): Borrowing at zero base rate sends debt request to user (790ms)
      ✓ withdrawSAI(): reverts when calling address does not have active trove (370ms)
      ✓ withdrawSAI(): reverts when requested withdrawal amount is zero SAI (459ms)
      ✓ withdrawSAI(): reverts when system is in Recovery Mode (649ms)
      ✓ withdrawSAI(): reverts when withdrawal would bring the trove's ICR < MCR (347ms)
      ✓ withdrawSAI(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR (439ms)
      ✓ withdrawSAI(): reverts if system is in Recovery Mode (312ms)
      ✓ withdrawSAI(): increases the Trove's SAI debt by the correct amount (208ms)
      ✓ withdrawSAI(): increases SAI debt in ActivePool by correct amount (230ms)
      ✓ withdrawSAI(): increases user SAIToken balance by correct amount (243ms)
      ✓ repaySAI(): reverts when repayment would leave trove with ICR < MCR (392ms)
      ✓ repaySAI(): Succeeds when it would leave trove with net debt >= minimum net debt (435ms)
      ✓ repaySAI(): reverts when it would leave trove with net debt < minimum net debt (256ms)
      ✓ repaySAI(): reverts when calling address does not have active trove (431ms)
      ✓ repaySAI(): reverts when attempted repayment is > the debt of the trove (418ms)
      ✓ repaySAI(): reduces the Trove's SAI debt by the correct amount (403ms)
      ✓ repaySAI(): decreases SAI debt in ActivePool by correct amount (406ms)
      ✓ repaySAI(): decreases user SAIToken balance by correct amount (404ms)
      ✓ repaySAI(): can repay debt in Recovery Mode (557ms)
      ✓ repaySAI(): Reverts if borrower has insufficient SAI balance to cover his debt repayment (1122ms)
      ...
      ✓ adjustTrove(): borrowing at non-zero base rate sends SAI fee to LQTY staking contract (889ms)
      ✓ adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (1013ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate increases the LQTY staking contract SAI fees-per-unit-staked (837ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user (895ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes SAI balance of LQTY staking contract (783ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes LQTY staking contract SAI fees-per-unit-staked (881ms)
      ✓ adjustTrove(): Borrowing at zero base rate sends total requested SAI to the user (783ms)
      ...
      ✓ adjustTrove(): reverts when SAI repaid is > debt of the trove (451ms)
      ...
      ✓ adjustTrove(): changes SAIToken balance by the requested decrease (529ms)
      ✓ adjustTrove(): changes SAIToken balance by the requested increase (577ms)
      ...
      ✓ adjustTrove(): Changes the SAI debt in ActivePool by requested decrease (541ms)
      ✓ adjustTrove(): Changes the SAI debt in ActivePool by requested increase (397ms)
      ...
      ✓ adjustTrove(): Reverts if borrower has insufficient SAI balance to cover his debt repayment (496ms)
      ...
      ✓ closeTrove(): subtracts the debt of the closed Trove from the Borrower's SAIToken balance (405ms)
      ✓ closeTrove(): applies pending rewards (1267ms)
      ✓ closeTrove(): reverts if borrower has insufficient SAI balance to repay his entire debt (347ms)
      ...
      ✓ openTrove(): borrowing at non-zero base rate sends SAI fee to LQTY staking contract (923ms)
      ✓ openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct (865ms)
      ✓ openTrove(): Borrowing at non-zero base rate increases the LQTY staking contract SAI fees-per-unit-staked (794ms)
      ✓ openTrove(): Borrowing at non-zero base rate sends requested amount to the user (832ms)
      ✓ openTrove(): Borrowing at zero base rate changes the LQTY staking contract SAI fees-per-unit-staked (602ms)
      ...
      ✓ openTrove(): records up-to-date initial snapshots of L_ETH and L_SAIDebt (639ms)
      ✓ openTrove(): allows a user to open a Trove, then close it, then re-open it (687ms)
      ✓ openTrove(): increases the Trove's SAI debt by the correct amount (129ms)
      ✓ openTrove(): increases SAI debt in ActivePool by the debt of the trove (162ms)
      ✓ openTrove(): increases user SAIToken balance by correct amount (85ms)
  ...
  Contract: Deployment script - Sets correct contract addresses dependencies after deployment
    ...
    ✓ Sets the correct SAIToken address in TroveManager
    ...
    ✓ Sets the correct SAIToken address in StabilityPool
...
TCR: 11.892415157517211309
  ...
  Contract: Deploying the LQTY contracts: LCF, CI, LQTYStaking, and LQTYToken 
    ...
    Community Issuance deployment
      ...
      ✓ Fluid AG can set addresses if CI's LQTY balance is equal or greater than 32 million  (393ms)
      ✓ Fluid AG can't set addresses if CI's LQTY balance is < 32 million  (367ms)
  ...
  Contract: During the initial lockup period
    LQTY transfer during first year after LQTY deployment
      ✓ Fluid multisig can not transfer LQTY to a LC that was deployed directly (105ms)
      ✓ Fluid multisig can not transfer to an EOA or Fluid system contracts (302ms)
      ✓ Fluid multisig can not approve any EOA or Fluid system contract to spend their LQTY (546ms)
      ✓ Fluid multisig can not increaseAllowance for any EOA or Fluid contract (285ms)
      ✓ Fluid multisig can not decreaseAllowance for any EOA or Fluid contract (338ms)
      ✓ Fluid multisig can not be the sender in a transferFrom() call
      ✓ Fluid multisig can not stake their LQTY in the staking contract
      ✓ Anyone (other than Fluid multisig) can transfer LQTY to LCs deployed by anyone through the Factory (359ms)
      ✓ Anyone (other than Fluid multisig) can transfer LQTY to LCs deployed by anyone directly (141ms)
      ✓ Anyone (other than fluid multisig) can transfer to an EOA (106ms)
      ✓ Anyone (other than fluid multisig) can approve any EOA or to spend their LQTY
      ✓ Anyone (other than fluid multisig) can increaseAllowance for any EOA or Fluid contract (244ms)
      ✓ Anyone (other than fluid multisig) can decreaseAllowance for any EOA or Fluid contract (693ms)
      ✓ Anyone (other than fluid multisig) can be the sender in a transferFrom() call (62ms)
      ✓ Anyone (other than fluid AG) can stake their LQTY in the staking contract
    ...
    Transferring LQTY to LCs
      ✓ Fluid multisig can transfer LQTY (vesting) to lockup contracts they deployed (198ms)
      ✓ Fluid multisig can transfer LQTY to lockup contracts deployed by anyone (1798ms)
    Deploying new LCs
      ✓ LQTY Deployer can deploy LCs through the Factory (46ms)
      ✓ Fluid multisig can deploy LCs through the Factory
      ✓ Anyone can deploy LCs through the Factory (80ms)
      ✓ LQTY Deployer can deploy LCs directly (44ms)
      ✓ Fluid multisig can deploy LCs directly (45ms)
      ...
      Withdrawal Attempts on LCs before unlockTime has passed 
        ✓ Fluid multisig can't withdraw from a funded LC they deployed for another beneficiary through the Factory before the unlockTime
        ✓ Fluid multisig can't withdraw from a funded LC that someone else deployed before the unlockTime (52ms)
  ...
  Contract: After the initial lockup period has passed
    ...
    Transferring LQTY
      ...
      ✓ Anyone can increaseAllowance for any EOA or Fluid contract (2441ms)
      ✓ Anyone can decreaseAllowance for any EOA or Fluid contract (1034ms)
...
issuance fraction after: 949066037374286
  ...
  Contract: LQTYStaking revenue share tests
    ...
    ✓ SAI fee per LQTY staked increases when a redemption fee is triggered and totalStakes > 0 (1757ms)
    ✓ SAI fee per LQTY staked doesn't change when a redemption fee is triggered and totalStakes == 0 (1705ms)
    ✓ LQTY Staking: A single staker earns all ETH and LQTY fees that occur (2143ms)
    ✓ stake(): Top-up sends out all accumulated ETH and SAI gains to the staker (2138ms)
    ✓ getPendingETHGain(): Returns the staker's correct pending ETH gain (1856ms)
    ✓ getPendingSAIGain(): Returns the staker's correct pending SAI gain (1965ms)
  ...
  Contract: All Fluid functions with onlyOwner modifier
    TroveManager
  ...
  Contract: StabilityPool
    ...
    ✓ getTotalSAIDeposits(): gets the recorded SAI balance

  Contract: ActivePool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getSAIDebt(): gets the recorded SAI balance
    ✓ increaseSAI(): increases the recorded SAI balance by the correct amount
    ✓ decreaseSAI(): decreases the recorded SAI balance by the correct amount
    ✓ sendETH(): decreases the recorded ETH balance by the correct amount

  Contract: DefaultPool
    ✓ getETH(): gets the recorded SAI balance
    ✓ getSAIDebt(): gets the recorded SAI balance
    ✓ increaseSAI(): increases the recorded SAI balance by the correct amount
    ✓ decreaseSAI(): decreases the recorded SAI balance by the correct amount (57ms)
  ...
  Contract: BorrowerWrappers
    ...
    ✓ claimStakingGainsAndRecycle(): with only SAI gain (821ms)
    ✓ claimStakingGainsAndRecycle(): with both ETH and SAI gains (1593ms)
...
totalLQTYIssued_2: 30370113195977152000000
      ...
      ✓ withdrawFromSP(): Several deposits of 100 SAI span one scale factor change. Depositors withdraw correct LQTY gains (6075ms)
      ...
      ✓ withdrawFromSP(): Several deposits of 10k SAI span one scale factor change. Depositors withdraw correct LQTY gains (4788ms)

  Contract: Pool Manager: Sum-Product rounding errors
    - Rounding errors: 100 deposits of 100SAI into SP, then 200 liquidations of 49SAI
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing SAI (965ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing SAI (1152ms)
      ...
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 SAI.  A, B, C, D withdraw correct SAI deposit and ETH Gain (1512ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 SAI.  A, B, C, D withdraw correct SAI deposit and ETH Gain (1637ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct SAI deposit and ETH Gain (1645ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 SAI.  A, B, C, D withdraw correct SAI deposit and ETH Gain (1704ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct SAI deposit and ETH Gain (1753ms)
      ✓ withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 SAI. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct SAI deposit and ETH Gain (1537ms)
...
alice deposit: 0
      ...
      ✓ withdrawFromSP(): Several deposits of 10000 SAI span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (1676ms)
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing SAI (1597ms)
      ✓ withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing SAI (1711ms)
      ...
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 SAI.  A, B, C, D withdraw correct SAI deposit and ETH Gain (2168ms)
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 SAI.  A, B, C, D withdraw correct SAI deposit and ETH Gain (5085ms)
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct SAI deposit and ETH Gain (2304ms)
      ✓ withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 SAI.  A, B, C, D withdraw correct SAI deposit and ETH Gain (2335ms)
      ✓ withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct SAI deposit and ETH Gain (2145ms)
      ✓ withdrawETHGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 SAI. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct SAI deposit and ETH Gain (2543ms)
...
alice deposit: 0
      ...
      ✓ withdrawETHGainToTrove(): Several deposits of 10000 SAI span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (2361ms)
  ...
  Contract: StabilityPool
    Stability Pool Mechanisms
      ✓ provideToSP(): increases the Stability Pool SAI balance (354ms)
      ✓ provideToSP(): updates the user's deposit record in StabilityPool (355ms)
      ✓ provideToSP(): reduces the user's SAI balance by the correct amount (321ms)
      ✓ provideToSP(): increases totalSAIDeposits by correct amount (283ms)
      ...
      ✓ provideToSP(): reverts if user tries to provide more than their SAI balance (1030ms)
      ✓ provideToSP(): reverts if user tries to provide 2^256-1 SAI, which exceeds their balance (508ms)
      ...
      ✓ provideToSP(): providing 0 SAI reverts (1091ms)
      ...
      ✓ withdrawFromSP(): partial retrieval - retrieves correct SAI amount and the entire ETH Gain, and updates deposit (1102ms)
      ✓ withdrawFromSP(): partial retrieval - leaves the correct amount of SAI in the Stability Pool (1128ms)
      ✓ withdrawFromSP(): full retrieval - leaves the correct amount of SAI in the Stability Pool (1091ms)
      ...
      ✓ withdrawFromSP(): it correctly updates the user's SAI and ETH snapshots of entitled reward per unit staked (1049ms)
      ...
      ✓ withdrawFromSP(): increases depositor's SAI token balance by the expected amount (2098ms)
      ...
      ✓ withdrawFromSP(): withdrawing 0 SAI doesn't alter the caller's deposit or the total SAI in the Stability Pool (993ms)
      ...
      ✓ withdrawFromSP(): Request to withdraw 2^256-1 SAI only withdraws the caller's compounded deposit (1229ms)
      ...
      ✓ withdrawETHGainToTrove(): Applies SAILoss to user's deposit, and redirects ETH reward to user's Trove (932ms)
...
B stake after A11: 39999999999999999998008332745
  ...
  Contract: TroveManager - in Recovery Mode
    ...
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: offsets the trove entirely with the pool (1265ms)
    ✓ liquidate(), with ICR% = 110 < TCR, and StabilityPool SAI > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus (1382ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: removes stake and updates totalStakes (1361ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: updates system snapshots (1287ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: closes the Trove (6532ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: can liquidate troves out of order (2667ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: Trove remains active (985ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: Trove remains in TroveOwners array (1134ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: nothing happens (1146ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: updates system shapshots (1284ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves (1266ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool SAI < liquidated debt: ICR of non liquidated trove does not change (2161ms)
    ✓ liquidate() with ICR > 110%, and StabilityPool SAI < liquidated debt: total liquidated coll and debt is correct (1912ms)
...
gasUsed:  636956
    ...
    ✓ batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool SAI > debt to liquidate: can liquidate troves out of order (1357ms)
  ...
  Contract: TroveManager
    ...
    ✓ liquidate(): decreases ActivePool ETH and SAIDebt by correct amounts (567ms)
    ✓ liquidate(): increases DefaultPool ETH and SAI debt by correct amounts (553ms)
    ...
    ✓ liquidate(): updates the L_ETH and L_SAIDebt reward-per-unit-staked totals (984ms)
    ...
    ✓ redeemCollateral(): cancels the provided SAI with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether (838ms)
    ...
    ✓ redeemCollateral(): caller can redeem their entire SAIToken balance (1086ms)
    ✓ redeemCollateral(): reverts when requested redemption amount exceeds caller's SAI token balance (1320ms)
    ✓ redeemCollateral(): value of issued ETH == face value of redeemed SAI (assuming 1 SAI has value of $1) (1353ms)
    ...
    ✓ getPendingSAIDebtReward(): Returns 0 if there is no pending SAIDebt reward (618ms)
    ✓ getPendingETHReward(): Returns 0 if there is no pending ETH reward (620ms)
    ✓ computeICR(): Returns 0 if trove's coll is worth 0
    ✓ computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 SAI
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 SAI
    ✓ computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 SAI
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 SAI
