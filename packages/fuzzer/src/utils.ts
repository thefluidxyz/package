...
import { EthersLiquity, ReadableEthersLiquity } from "@fluid/lib-ethers";
import { SubgraphLiquity } from "@fluid/lib-subgraph";
...
export const getListOfTroves = async (liquity: ReadableFluid) =>
  liquity.getTroves({
...
export const getListOfTrovesBeforeRedistribution = async (liquity: ReadableFluid) =>
  liquity.getTroves({
    first: await liquity.getNumberOfTroves(),
    sortedBy: "descendingCollateralRatio",
...
export const getListOfTroveOwners = async (liquity: ReadableFluid) =>
...
class EqualityCheck<T> {
  private name: string;
  private get: (l: ReadableFluid) => Promise<T>;
  private equals: (a: T, b: T) => boolean;

  constructor(
    name: string,
    get: (l: ReadableFluid) => Promise<T>,
  ...
  async allEqual(liquities: ReadableFluid[]) {
...
export const checkSubgraph = async (subgraph: SubgraphLiquity, l1Liquity: ReadableFluid) => {
