...
export interface FluidStoreBaseState {
   ...
   *
   * @remarks
   * See {@link ReadableFluid.getCollateralSurplusBalance | getCollateralSurplusBalance()} for
   * more information.
   */
   ...
   * {@link FluidStoreDerivedState.trove | trove}.
   */
...
/**
 * State variables derived from {@link FluidStoreBaseState}.
 *
 * @public
 */
...
/**
 * Type of {@link FluidStore}'s {@link FluidStore.state | state}.
 *
 * @remarks
 * It combines all properties of {@link FluidStoreBaseState} and {@link FluidStoreDerivedState}
 * with optional extra state added by the particular `FluidStore` implementation.
 *
...
/**
 * Parameters passed to {@link FluidStore} listeners.
 *
 * @remarks
 * Use the {@link FluidStore.subscribe | subscribe()} function to register a listener.
 ...
 *
 * @remarks
 * The type parameter `T` may be used to type extra state added to {@link FluidStoreState} by the
 * subclass.
 *
 * Implemented by {@link @fluid/lib-ethers#BlockPolledFluidStore}.
 *
 * @public
 */
export abstract class FluidStore<T = unknown> {
   ...
   *
   * @remarks
   * See {@link FluidStore.start | start()}.
   */
  onLoaded?: () => void;
  ...
  private _baseState?: FluidStoreBaseState;
  private _derivedState?: FluidStoreDerivedState;
  private _extraState?: T;

  private _updateTimeoutId: ReturnType<typeof setTimeout> | undefined;
  private _listeners = new Set<(params: FluidStoreListenerParams<T>) => void>();
   ...
   * {@link FluidStore.onLoaded | onLoaded} to get a callback when this happens.
   *
   * See {@link FluidStoreState} for the list of properties returned.
   */
   ...
   *
   * @remarks
   * The {@link FluidStore.onLoaded | onLoaded} callback will be called after the state is fetched
   * for the first time.
   *
   * Use the {@link FluidStore.subscribe | subscribe()} function to register listeners.
   *
  ...
  private _reduce(
    baseState: FluidStoreBaseState,
    baseStateUpdate: Partial<FluidStoreBaseState>
  ): FluidStoreBaseState {
    return {
  ...
  }: FluidStoreBaseState): FluidStoreDerivedState {
  ...
  private _reduceDerived(
    derivedState: FluidStoreDerivedState,
    derivedStateUpdate: FluidStoreDerivedState
  ): FluidStoreDerivedState {
    return {
  ...
  private _notify(params: FluidStoreListenerParams<T>) {
