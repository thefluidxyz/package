 ...
 *
 * @remarks
 * Returned by the {@link ReadableFluid.getFees | getFees()} function.
 *
 * @public
 */
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in SAI, multiply the borrowed SAI amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await liquity.getFees();
   *
   * const borrowedSAIAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeSAI = borrowingRate.mul(borrowedSAIAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of SAI being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total SAI supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in SAI, multiply the redeemed SAI amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedSAIAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedSAIAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeSAI = redemptionRate.mul(redeemedSAIAmount);
   * ```
   */
