...
import { BlockPolledLiquityStoreState } from "@fluid/lib-ethers";
...
const liquidatableInRecoveryMode = (
  ...
  if (collateralRatio.gte(MINIMUM_COLLATERAL_RATIO) && collateralRatio.lt(totalCollateralRatio)) {
    return [
      trove.debt.lte(saiInStabilityPool),
      "There's not enough SAI in the Stability pool to cover the debt"
    ] as const;
  } else {
...
export const RiskyTroves: React.FC<RiskyTrovesProps> = ({ pageSize }) => {
  ...
  }, [fluid, clampedPage, pageSize, reload]);
                    ...
                    <tr key={trove.ownerAddress}>
                      ...
                      <td>
                        <Transaction
                          ...
                          send={fluid.send.liquidate.bind(fluid.send, trove.ownerAddress)}
                        >
