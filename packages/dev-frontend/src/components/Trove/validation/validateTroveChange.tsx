...
const TroveChangeDescription: React.FC<TroveAdjustmentDescriptionParams> = ({ params }) => (
  <ActionDescription>
    {params.depositCollateral && params.borrowSAI ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} SEI</Amount> and receive{" "}
        <Amount>
          {params.borrowSAI.prettify()} {COIN}
        </Amount>
      </>
    ) : params.repaySAI && params.withdrawCollateral ? (
      <>
        You will pay{" "}
        <Amount>
          {params.repaySAI.prettify()} {COIN}
        </Amount>{" "}
        and receive <Amount>{params.withdrawCollateral.prettify()} SEI</Amount>
      </>
    ) : params.depositCollateral && params.repaySAI ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} SEI</Amount> and pay{" "}
        <Amount>
          {params.repaySAI.prettify()} {COIN}
        </Amount>
      </>
    ) : params.borrowSAI && params.withdrawCollateral ? (
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify()} SEI</Amount> and{" "}
        <Amount>
          {params.borrowSAI.prettify()} {COIN}
        </Amount>
      </>
      ...
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} SEI</Amount>
      </>
    ) : params.withdrawCollateral ? (
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify()} SEI</Amount>
      </>
    ) : params.borrowSAI ? (
      <>
        You will receive{" "}
...
export const selectForTroveChangeValidation = ({
  ...
  saiBalance,
  numberOfTroves
}: FluidStoreState) => ({ price, total, accountBalance, saiBalance, numberOfTroves });
  ...
  if (change.type === "invalidCreation") {
    ...
    return [
      ...
      <ErrorDescription>
        ...
        <Amount>
          {SAI_MINIMUM_DEBT.toString()} {COIN}
        </Amount>
        .
      </ErrorDescription>
    ];
...
const validateTroveCreation = (
  { depositCollateral, borrowSAI }: TroveCreationParams<Decimal>,
  {
    resultingTrove,
    recoveryMode,
    wouldTriggerRecoveryMode,
  ...
  if (borrowSAI.lt(SAI_MINIMUM_NET_DEBT)) {
    return (
      <ErrorDescription>
        You must borrow at least{" "}
        <Amount>
          {SAI_MINIMUM_NET_DEBT.toString()} {COIN}
        </Amount>
        .
      </ErrorDescription>
    );
  ...
  if (depositCollateral.gt(accountBalance)) {
    return (
      <ErrorDescription>
        ...
        <Amount>{depositCollateral.sub(accountBalance).prettify()} SEI</Amount>.
      </ErrorDescription>
    );
  }
...
const validateTroveAdjustment = (
  { depositCollateral, withdrawCollateral, borrowSAI, repaySAI }: TroveAdjustmentParams<Decimal>,
  {
    originalTrove,
    resultingTrove,
    recoveryMode,
  ...
  if (repaySAI) {
    if (resultingTrove.debt.lt(SAI_MINIMUM_DEBT)) {
      return (
        <ErrorDescription>
          Total debt must be at least{" "}
          <Amount>
    ...
    if (repaySAI.gt(saiBalance)) {
      return (
        <ErrorDescription>
          The amount you're trying to repay exceeds your balance by{" "}
          <Amount>
            {repaySAI.sub(saiBalance).prettify()} {COIN}
          </Amount>
          .
        </ErrorDescription>
      );
...
const validateTroveClosure = (
  { repaySAI }: TroveClosureParams<Decimal>,
  {
    recoveryMode,
    wouldTriggerRecoveryMode,
    numberOfTroves,
