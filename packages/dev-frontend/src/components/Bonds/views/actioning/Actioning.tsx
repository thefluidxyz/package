    ...
    },
    {
      ...
      label: (
        <>
          ...
          <SubLabel style={{ fontWeight: 400 }}>{`${bond.accrued.prettify(2)} bSAI`}</SubLabel>
        </>
      ),
      isEndOfLife: true,
      isMilestone: false
  ...
  return (
    <ReactModal onDismiss={handleDismiss}>
      ...
      <Grid gap="12px" columns={3} sx={{ my: 4, justifyItems: "center" }}>
        ...
        <Record lexicon={l.MARKET_VALUE} value={bond.marketValue.prettify(2)} type="SAI" />

        {view === "CLAIMING" && (
          <Record lexicon={l.BOND_RETURN} value={bond.claimNowReturn.toFixed(2)} type="SAI" />
        )}
      </Grid>
      <details>
